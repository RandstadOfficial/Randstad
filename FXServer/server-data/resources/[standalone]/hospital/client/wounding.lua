            ...
            if bleedTickTimer >= Config.BleedTickRate and not isInHospitalBed then
                if not isDead and not InLaststand then
                    if isBleeding > 0 then
                        ...
                        TriggerServerEvent("evidence:server:CreateBloodDrop", RSCore.Functions.GetPlayerData().citizenid, RSCore.Functions.GetPlayerData().metadata["bloodtype"], coords)
...
AddEventHandler('hospital:client:UseBandage', function()
    RSCore.Functions.Progressbar("use_bandage", "Verband omdoen..", 5000, false, true, {
        ...
        TriggerServerEvent("RSCore:Server:RemoveItem", "bandage", 1)
        TriggerEvent("inventory:client:ItemBox", RSCore.Shared.Items["bandage"], "remove")
...
AddEventHandler('hospital:client:UsePainkillers', function()
    RSCore.Functions.Progressbar("use_bandage", "Pijnstillers innemen", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
