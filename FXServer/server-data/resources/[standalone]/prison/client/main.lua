		...
		if isLoggedIn then
			if inJail then
				...
				if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config.Locations["freedom"].coords.x, Config.Locations["freedom"].coords.y, Config.Locations["freedom"].coords.z, true) < 1.5) then
					RSCore.Functions.DrawText3D(Config.Locations["freedom"].coords.x, Config.Locations["freedom"].coords.y, Config.Locations["freedom"].coords.z, "~g~E~w~ - Check tijd")
...
AddEventHandler('prison:client:Enter', function(time)
	RSCore.Functions.Notify("Je zit in de gevangenis voor "..time.." maanden..", "error")
