	...
	elseif hasToPay then
		RSCore.Functions.GetPlayerData(function(PlayerData)
			...
			if cashBalance >= 100 or cashBalance >= 100 then
				...
				RSCore.Functions.Notify("Je hebt niet genoeg geld! (€100,-)")
			end
		end)
	end
end
...
function ClothShop()
	...
	drawTxt(0.177, 0.066, 0.25, 0.03, 0.40,"Kledingwinkel",255,255,255,255) -- header
	...
	elseif cmenu.show == 2 then
		...
		if GetNumberOfPedDrawableVariations(GetPlayerPed(-1), cmenu.field-1) ~= 0 and GetNumberOfPedDrawableVariations(GetPlayerPed(-1), cmenu.field-1) ~= false then
			...
			drawTxt(0.328,0.204,0.175,0.035, 0.40,"Niet beschikbaar",255,255,255,255)
			if IsControlJustPressed(1, 189) or IsDisabledControlJustPressed(1, 189) then -- left
				PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
				if cmenu.field > 1 then 
					cmenu.field = cmenu.field-1
	...
	elseif cmenu.show == 3 then
		...
		local drawstr = string.format("Type: %d | Kleur: %d",prop[1],prop[2])
	...
	elseif cmenu.show == 4 then
		...
		drawTxt(0.328,0.093+0.037*2,0.175,0.035, 0.40,"Zoek op naam",255,255,255,255)
		if IsControlJustPressed(1, 189) or IsDisabledControlJustPressed(1, 189) then -- left
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if nearcloth < 5.0 and cmenu.show == 0 and RSCore ~= nil then
			local pos = GetEntityCoords(GetPlayerPed(-1))
			RSCore.Functions.DrawText3D(pos.x, pos.y, pos.z, "~g~E~w~ - om kleding aan te passen / ~g~H~w~ - Om outfit op te slaan")
