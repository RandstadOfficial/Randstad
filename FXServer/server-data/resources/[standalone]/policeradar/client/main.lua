        ...
        if PlayerData.job.name == "police" then
            ...
            if (IsPedInAnyVehicle( ped ) ) then 
                ...
                if (IsPoliceVehicle(vehicle)) then
                    ...
                    RSCore.Functions.Notify( "Je moet in een politie voertuig zitten!", "error")
                end 
            else 
                RSCore.Functions.Notify( "Je moet in een voertuig zitten!", "error")
            end 
        end
    end)
end)
...
AddEventHandler( 'wk:changeRadarLimit', function( speed ) 
    ...
    RSCore.Functions.Notify("Max snelheid gezet op: " .. speed .. "km/u")
end )
                        ...
                        if plateChecked ~= fwdPlate then
                            RSCore.Functions.TriggerCallback('police:IsPlateFlagged', function(result)
                                if result then
                                    ...
                                    radarInfo.fwdFastLocked = true 
                                    SendNUIMessage( { lockfwdfast = true } )
                                    RSCore.Functions.Notify("Gemarkeerd voertuig gevonden!", "error")
                                end
    ...
    while true do 
        ...
        if RSCore ~= nil then 
            ...
            if ( IsDisabledControlPressed( 0, Keys["LEFTCTRL"] ) and IsDisabledControlJustPressed( 0, Keys["7"] ) ) then 
                if RSCore.Functions.GetPlayerData().job.name == "police" and RSCore.Functions.GetPlayerData().job.onduty then
            ...
            if ( not IsDisabledControlPressed( 0, Keys["LEFTCTRL"] ) and IsDisabledControlJustPressed( 0, Keys["7"] ) ) then 
                if RSCore.Functions.GetPlayerData().job.name == "police" and RSCore.Functions.GetPlayerData().job.onduty then
                    ...
                    ResetFrontAntenna()
                    ResetRearAntenna()
                    RSCore.Functions.Notify("Politie ANPR gereset")
                end
            end 
            ...
            if ( IsDisabledControlJustPressed( 0, Keys["8"] ) ) then 
                if RSCore.Functions.GetPlayerData().job.name == "police" and RSCore.Functions.GetPlayerData().job.onduty then
                    ...
                    if (radarInfo.plateLocked) then
                        ...
                        RSCore.Functions.Notify("Kenteken locked: " .. radarInfo.fwdPlate)
                    else
                        SendNUIMessage( { unlockPlate = true } )
                        RSCore.Functions.Notify("Kenteken unlocked")
                    end
                end
            end
