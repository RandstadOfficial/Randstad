...
RSCore.Commands.Refresh = function(source)
	local Player = RSCore.Functions.GetPlayer(tonumber(source))
	if Player ~= nil then
		for command, info in pairs(RSCore.Commands.List) do
...
RSCore.Commands.Add("tp", "Teleport naar een speler of location", {{name="id/x", help="ID van een speler of X positie"}, {name="y", help="Y positie"}, {name="z", help="Z positie"}}, false, function(source, args)
	if (args[1] ~= nil and (args[2] == nil and args[3] == nil)) then
		-- tp to player
		local Player = RSCore.Functions.GetPlayer(tonumber(args[1]))
		if Player ~= nil then
			TriggerClientEvent('RSCore:Command:TeleportToPlayer', source, Player.PlayerData.source)
		else
		...
		if args[1] ~= nil and args[2] ~= nil and args[3] ~= nil then
			...
			TriggerClientEvent('RSCore:Command:TeleportToCoords', source, x, y, z)
		else
...
RSCore.Commands.Add("addpermission", "Geef permissie aan iemand (god/admin)", {{name="id", help="ID van de speler"}, {name="permission", help="Permission level"}}, true, function(source, args)
	local Player = RSCore.Functions.GetPlayer(tonumber(args[1]))
	local permission = tostring(args[2]):lower()
	if Player ~= nil then
		RSCore.Functions.AddPermission(Player.PlayerData.source, permission)
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Speler is niet online!")	
	end
end, "god")

RSCore.Commands.Add("removepermission", "Haal permissie weg van iemand", {{name="id", help="ID van de speler"}}, true, function(source, args)
	local Player = RSCore.Functions.GetPlayer(tonumber(args[1]))
	if Player ~= nil then
		RSCore.Functions.RemovePermission(Player.PlayerData.source)
	else
		TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Speler is niet online!")	
	end
end, "god")

RSCore.Commands.Add("sv", "Spawn een voertuig", {{name="model", help="Model naam van het voertuig"}}, true, function(source, args)
	TriggerClientEvent('RSCore:Command:SpawnVehicle', source, args[1])
end, "admin")
...
RSCore.Commands.Add("tpm", "Teleport naar een marker", {}, false, function(source, args)
	TriggerClientEvent('RSCore:Command:GoToMarker', source)
end, "admin")

RSCore.Commands.Add("givemoney", "Geef geld aan een speler", {{name="id", help="Speler ID"},{name="moneytype", help="Type geld (cash, bank, crypto)"}, {name="amount", help="Aantal munnies"}}, true, function(source, args)
	local Player = RSCore.Functions.GetPlayer(tonumber(args[1]))
	if Player ~= nil then
...
RSCore.Commands.Add("clearinv", "Leeg de inventory van jezelf of een speler", {{name="id", help="Speler ID"}}, false, function(source, args)
	...
	local Player = RSCore.Functions.GetPlayer(tonumber(playerId))
	if Player ~= nil then
