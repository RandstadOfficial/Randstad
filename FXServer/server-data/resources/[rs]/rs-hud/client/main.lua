    ...
    while true do
        ...
        if (PlayerJob.name ~= "police") then
            if IsPlayerFreeAiming(PlayerId()) and not IsPedShooting(GetPlayerPed(-1)) then
                ...
                local WeaponData = RSCore.Shared.Weapons[CurrentWeapon]
    ...
    while true do
        if not IsGaining then
            ...
            if StressGain > 0 then
                RSCore.Functions.Notify('Stress opgedaan', "primary", 2000)
                TriggerServerEvent('rs-hud:Server:UpdateStress', StressGain)
                StressGain = 0
            end
        end
...
function GetShakeIntensity(stresslevel)
    local retval = 0.05
    for k, v in pairs(RSStress.Intensity["shake"]) do
...
function GetEffectInterval(stresslevel)
    local retval = 60000
    for k, v in pairs(RSStress.EffectInterval) do
        if stresslevel >= v.min and stresslevel < v.max then
            retval = v.timeout
            break
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        elseif stress >= RSStress.MinimumStress then
            local ShakeIntensity = GetShakeIntensity(stress)
            ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', ShakeIntensity)
