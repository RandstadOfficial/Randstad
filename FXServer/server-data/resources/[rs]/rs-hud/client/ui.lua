...
Citizen.CreateThread(function()
    Citizen.Wait(500)
    while true do 
        if RSCore ~= nil and isLoggedIn and RSHud.Show then
    ...
    while true do
        if RSCore ~= nil and isLoggedIn and RSHud.Show then
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                speed = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 3.6
                if speed >= RSStress.MinimumSpeed then
...
Citizen.CreateThread(function() 
    while true do
        Citizen.Wait(1000)
        if IsPedInAnyVehicle(PlayerPedId()) and isLoggedIn and RSHud.Show then
            DisplayRadar(true)
            SendNUIMessage({
                action = "car",
                show = true,
...
RegisterNetEvent('rs-hud:client:ToggleHarness')
AddEventHandler('rs-hud:client:ToggleHarness', function(toggle)
    SendNUIMessage({
        action = "harness",
        toggle = toggle
    })
...
RegisterNetEvent('rs-hud:client:UpdateNitrous')
AddEventHandler('rs-hud:client:UpdateNitrous', function(toggle, level, IsActive)
    SendNUIMessage({
        action = "nitrous",
        toggle = toggle,
        level = level,
...
RegisterNetEvent('rs-hud:client:UpdateDrivingMeters')
AddEventHandler('rs-hud:client:UpdateDrivingMeters', function(toggle, amount)
    SendNUIMessage({
...
RegisterNetEvent('rs-hud:client:UpdateVoiceProximity')
AddEventHandler('rs-hud:client:UpdateVoiceProximity', function(Proximity)
    SendNUIMessage({
        action = "proximity",
        prox = Proximity
    })
...
RegisterNetEvent('rs-hud:client:ProximityActive')
AddEventHandler('rs-hud:client:ProximityActive', function(active)
    SendNUIMessage({
        action = "talking",
        IsTalking = active
    })
...
Citizen.CreateThread(function()
    while true do
        if isLoggedIn and RSHud.Show and RSCore ~= nil then
            RSCore.Functions.TriggerCallback('hospital:GetPlayerBleeding', function(playerBleeding)
