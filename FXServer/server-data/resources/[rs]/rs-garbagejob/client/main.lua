        ...
        if isLoggedIn then
            if PlayerJob.name == "garbage" then
                if distance < 10.0 then
                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Vuilniswagen opbergen")
                            ...
                            DrawText3D(Config.Locations["vehicle"].coords.x, Config.Locations["vehicle"].coords.y, Config.Locations["vehicle"].coords.z, "~g~E~w~ - Vuilniswagen")
                            if IsControlJustReleased(0, Keys["E"]) then
                                RSCore.Functions.TriggerCallback('rs-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        RSCore.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            ...
                                            if IsControlJustPressed(0, 47) then
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        RSCore.Functions.Notify("Alle vuilniszakken zijn gedaan, ga door naar de volgende locatie!")
                                                    else
                                                        -- Hier ben je klaar met werken.
                                                        RSCore.Functions.Notify("Je bent klaar met werken! Ga terug naar de vuilnisbelt.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        RSCore.Functions.Notify("Er zijn nog "..AmountOfBags.." zakken over!")
                                                    else
                                                        RSCore.Functions.Notify("Er is nog "..AmountOfBags.." zak over!")
                                                    end
                                                    hasZak = false
                                                end
                                                DeliverAnim()
