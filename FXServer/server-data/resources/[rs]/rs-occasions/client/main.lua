            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        RSCore.Functions.TriggerCallback('rs-occasions:server:getVehicles', function(vehicles)
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, RSCore.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Prijs: €'..Config.OccasionSlots[i]["price"]..',-')
                                if Config.OccasionSlots[i]["owner"] == RSCore.Functions.GetPlayerData().citizenid then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    RSCore.Functions.TriggerCallback('rs-occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
                                ...
                                if IsDisabledControlJustPressed(0, Keys["7"]) then
                                    isConfirming = false
                                    currentVehicle = i
                                    TriggerServerEvent("rs-occasions:server:ReturnVehicle", Config.OccasionSlots[i])
                                end
...
RegisterNUICallback('buyVehicle', function()
    ...
    TriggerServerEvent('rs-occasions:server:buyVehicle', vehData)
end)

DoScreenFadeIn(250)

RegisterNetEvent('rs-occasions:client:BuyFinished')
AddEventHandler('rs-occasions:client:BuyFinished', function(model, plate, mods)
    ...
    RSCore.Functions.SpawnVehicle(model, function(veh)
        ...
        RSCore.Functions.Notify("Voertuig gekocht", "success", 2500)
...
RegisterNUICallback('sellVehicle', function(data)
    ...
    TriggerServerEvent('rs-occasions:server:sellVehicle', data.price, vehicleData)
...
function sellVehicleWait(price)
    ...
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    RSCore.Functions.Notify('Uw auto staat te koop voor, €'..price..',-', 'success')
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end

RegisterNetEvent('rs-occasion:client:refreshVehicles')
AddEventHandler('rs-occasion:client:refreshVehicles', function()
    if inRange then
        RSCore.Functions.TriggerCallback('rs-occasions:server:getVehicles', function(vehicles)
            occasionVehicles = vehicles
            despawnOccasionsVehicles()
