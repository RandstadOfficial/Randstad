	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				RSCore.Functions.Notify("Je bent buiten de range gegaan!")
			end
		end
	end
end)
...
RegisterNetEvent("rs-houses:client:decorate")
AddEventHandler("rs-houses:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
			...
			RSCore.Functions.Notify("Je moet de sleutels van het huis hebben!", "error")
		end
	else
...
RegisterNetEvent("rs-houses:server:sethousedecorations")
AddEventHandler("rs-houses:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		RSCore.Functions.TriggerCallback('rs-houses:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("rs-houses:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		RSCore.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end
