...
AddEventHandler("RSCore:Client:OnPlayerLoaded", function()
    TriggerServerEvent("rs-scrapyard:server:LoadVehicleList")
end)
...
RegisterNetEvent('rs-scapyard:client:setNewVehicles')
AddEventHandler('rs-scapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = RSCore.Shared.Vehicles[v]
		...
		RSCore.Functions.Notify("Je mag nu geen voertuigen slopen..", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	RSCore.Functions.Progressbar("scrap_vehicle", "Voertuig slopen..", scrapTime, false, true, {
