...
RegisterNetEvent('rs-tunerchip:server:TuneStatus')
AddEventHandler('rs-tunerchip:server:TuneStatus', function()
    local ped = GetPlayerPed(-1)
    ...
    RSCore.Functions.TriggerCallback('rs-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    RSCore.Functions.Notify('Tjoenertjip v1.05: Voertuig is gereset!', 'error')
end)

RegisterNetEvent('rs-tunerchip:client:openChip')
AddEventHandler('rs-tunerchip:client:openChip', function()
    local ped = GetPlayerPed(-1)
