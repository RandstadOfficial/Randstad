...
AddEventHandler('RSCore:Client:OnPlayerLoaded', function()
    RSCore.Functions.TriggerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
...
AddEventHandler('smallresource:client:LoadNitrous', function()
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            RSCore.Functions.Progressbar("use_nos", "NOS wordt aangesloten.", 1000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
            }, {}, {}, {}, function() -- Done
                TriggerEvent("inventory:client:ItemBox", RSCore.Shared.Items['nitrous'], "remove")
        ...
        RSCore.Functions.Notify('Je hebt al NOS actief..', 'error')
    end
end)
                ...
                if VehicleNitrous[Plate].hasnitro then
                    ...
                    if IsControlJustReleased(0, Keys["LEFTCTRL"]) then
                        if NitrousActivated then
                            ...
                            StopScreenEffect("RaceTurbo")
                            TriggerEvent('rs-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, false)
                            NitrousActivated = false
                        end
                    end
                end
