...
$(document).on('click', '#save', function(){
    $.post('http://rs-tunerchip/save', JSON.stringify({
...
RSTuner.Open = function() {
    $.post('http://rs-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
RSTuner.Close = function() {
    $('.container').fadeOut(250);
    $.post('http://rs-tunerchip/exit');
}