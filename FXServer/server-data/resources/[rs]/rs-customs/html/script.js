...
$(document).ready(function(){
    console.log("rs-customs: Succesfully loaded!");

    window.addEventListener('message', function(event){
        switch(event.data.action) {
            case "open":
                $(".randstad-customs").css({
                    "display":"block"
                });
            ...
            case "close":
                $(".menu-container").animate({
                    ...
                    $(".randstad-customs").css({"display":"none"});
                });
                MenuData.SubMenu = [];
...
function ChangeOption(type) {
    ...
    if (MenuData.CurrentModId !== "shoppingcart") {
        $.post('http://rs-customs/OnIndexChange', JSON.stringify({
...
function SetupMods(mods, price) {
    $(".menu-options").html("");
    $("#menu-name").html("Welkom bij Benny's Motorsports");
    $("#menu-help").html("Kies een menu");
    ...
    if (mods.length > 0) {
        ...
        if (price !== null) {
            MenuData.TotalOptions = MenuData.TotalOptions + 1;
            var ModElement = '<div class="menu-option" data-menuindex="' + (MenuData.TotalOptions) + '"><div class="menu-option-name">Repareer Voertuig <div class="menu-option-price">$ ' + price + '</div></div></div>';
...
function SubmitOption() {
    ...
    if (OptionData.id !== "repairvehicle") {
        if (OptionData.id !== "close-menu" && OptionData.id !== "shoppingcart" && OptionData.id !== "main-menu" && OptionData.id !== "buy-upgrades") {
            ...
            $.post('http://rs-customs/GetCurrentMod', JSON.stringify({
                data: OptionData
                ...
                $.post('http://rs-customs/GetCartItem', JSON.stringify({
                    data: checkdata,
                }), function(marketitem){
                    if (OptionData.id !== null && OptionData.id !== undefined) {
                                ...
                                if (mod.price !== null && mod.price !== undefined) {
                                    ...
                                    if (MenuData.CurrentModId == "respray" || MenuData.CurrentModId == "wheels" || MenuData.CurrentModId == "wheelaccessories" || installedmod > -1) {
                                        if (installedmod > -1 && i == installedmod + 1) {
                                            ...
                                            $.post('http://rs-customs/ToggleCartedItem', JSON.stringify({toggle: true}));
                                        } else if (mod.price == 0) {
                                            mod.bought = false;
                            ...
                            if (!OptionData.cart || OptionData.cart === undefined){
                                ...
                                if (CurrentEquiped.length > 0) {
                                    ...
                                    if (!$("[data-menuindex='" + MenuData.SelectedIndex + "']").hasClass('cart-option')){
                                        ...
                                        $.post('http://rs-customs/RemoveCartItem', JSON.stringify({
                                        ...
                                        $.post('http://rs-customs/AddItemToCart', JSON.stringify({
                                            ItemData: OptionData,
                                        }));
                                    } else {
                                        $(".cart-option").find('.menu-option-price').html(pricelabel);
                                ...
                                if (MenuData.CurrentMenu === "shoppingcart") {
                                    console.log('Verwijder cart item');
                                } else {
                                    var pricelabel
            ...
            $.post('http://rs-customs/SelectSound');
            $.post('http://rs-customs/PurchaseUpgrades');
            ...
            $("#menu-name").html("Mijn Winkelwagen");
            $("#menu-help").html("Totaal Prijs: $" + TotalCartPrice);
    
            MenuData.TotalOptions = MenuData.TotalOptions + 1;
            $(".menu-options").append('<div class="menu-option" data-menuindex="' + MenuData.TotalOptions + '"><div class="menu-option-name">Bevestig Setup<div class="menu-option-price">$' + TotalCartPrice + '</div></div></div>');
            $("[data-menuindex='" + MenuData.TotalOptions + "']").data('OptionData', {
                id: "buy-upgrades",
            });
            ...
            $.post('http://rs-customs/QuitSound');
            $(".menu-container").animate({
                left: -45+"vh",
            }, 300, function(){
                $(".randstad-customs").css({"display":"none"});
            ...
            MenuData.SubMenu = [];
            MenuData.CurrentMenu = 0;
            $.post('http://rs-customs/CloseMenu');
        }
    } else {
        $.post('http://rs-customs/CanRepairVehicle', JSON.stringify({
...
function Back() {
    $.post('http://rs-customs/BackSound');
    if (MenuData.CurrentMenu !== "shoppingcart") {
        if (MenuData.SubMenu[0] !== null && MenuData.SubMenu[0] !== undefined) {
            if (MenuData.CurrentMenu - 1 !== 0) {
                ...
                $.post('http://rs-customs/CheckIfCartedItem');
                $(".menu-options").html("");
                $("#menu-name").html("Welkom bij Benny's Motorsports");
                $("#menu-help").html("Kies een menu");
                MenuData.TotalOptions = 0;
            ...
            if (MenuData.CurrentMenu - 1 >= -1) {
                $.post('http://rs-customs/GetShoppingCart', JSON.stringify({}), function(ShoppingCart){
                    $(".menu-options").html("");
                    var ModElement;
                    if (ShoppingCart.buttons.length > 0) {
                        ModElement = '<div class="menu-option" data-menuindex="1"><div class="menu-option-name">Winkelwagen <div class="menu-option-price"><i class="fas fa-shopping-cart equiped-check"></i> ' + ShoppingCart.buttons.length + '</div></div></div>'+
                        '<div class="menu-option" data-menuindex="2"><div class="menu-option-name">Hoofdmenu</div></div>' +
                        '<div class="menu-option" data-menuindex="3"><div class="menu-option-name">Sluit Menu</div></div>';
                        ...
                        ModElement = '<div class="menu-option" data-menuindex="1"><div class="menu-option-name">Hoofdmenu</div></div>'+
                        '<div class="menu-option" data-menuindex="2"><div class="menu-option-name">Sluit Menu</div></div>';
                        $(".menu-options").append(ModElement);
