...
Citizen.CreateThread(function()
    while true do
        ...
        if not HasKey and IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) and RSCore ~= nil and not IsHotwiring then
            ...
            RSCore.Functions.DrawText3D(vehpos.x, vehpos.y, vehpos.z, "~g~H~w~ - Hotwire")
            SetVehicleEngineOn(veh, false, false, true)
            ...
            if GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= nil and GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= 0 then
                ...
                if driver ~= 0 and not IsPedAPlayer(driver) then
                    if IsEntityDead(driver) then
                        IsRobbing = true
                        RSCore.Functions.Progressbar("rob_keys", "Sleutels pakken..", 3000, false, true, {}, {}, {}, {}, function() -- Done
...
function LockVehicle()
    local veh = RSCore.Functions.GetClosestVehicle()
        ...
        if HasKey then
            ...
            if vehLockStatus == 1 then
                ...
                if(GetVehicleDoorLockStatus(veh) == 2)then
                    RSCore.Functions.Notify("Voertuig vergrendeld!")
                else
                    RSCore.Functions.Notify("Er gaat iets fout met het vergrendel systeem!")
                end
            else
                Citizen.Wait(750)
                ClearPedTasks(GetPlayerPed(-1))
                ...
                if(GetVehicleDoorLockStatus(veh) == 1)then
                    RSCore.Functions.Notify("Voertuig ontgrendeld!")
                else
                    RSCore.Functions.Notify("Er gaat iets fout met het vergrendel systeem!")
                end
            end
    ...
    if vehicle ~= nil and vehicle ~= 0 then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 1.5 then
            ...
            if (vehLockStatus > 1) then
                ...
                RSCore.Functions.Progressbar("lockpick_vehicledoor", "Deur openbreken..", lockpickTime, false, true, {
                    ...
                    if math.random(1, 100) <= 90 then
                        RSCore.Functions.Notify("Deur open!")
                        TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 5, "unlock", 0.3)
...
function Hotwire()
    if not HasKey then 
        ...
        PoliceCall()
        RSCore.Functions.Progressbar("hotwire_vehicle", "Bezig met het contactslot", hotwireTime, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            if (math.random(0, 100) < 10) then
                HasKey = true
                RSCore.Functions.Notify("Hotwire gelukt!")
            else
                HasKey = false
                SetVehicleEngineOn(veh, false, false, true)
                RSCore.Functions.Notify("Hotwire mislukt!", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
