...
AddEventHandler('weapons:server:SaveWeaponAmmo', function()
    ...
    if Player ~= nil then
        RSCore.Functions.ExecuteSql(false, "SELECT * FROM `playerammo` WHERE `citizenid` = '".. Player.PlayerData.citizenid.."'", function(result)
            if result[1] == nil then
                RSCore.Functions.ExecuteSql(false, "INSERT INTO `playerammo` (`citizenid`, `ammo`) VALUES ('"..Player.PlayerData.citizenid.."', '"..json.encode(WeaponAmmo[Player.PlayerData.citizenid]).."')")
            else
                RSCore.Functions.ExecuteSql(false, "UPDATE `playerammo` SET ammo='"..json.encode(WeaponAmmo[Player.PlayerData.citizenid]).."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'")
            end
        end)
    end
end)

RSCore.Functions.CreateCallback("weapon:server:GetWeaponAmmo", function(source, cb, ammotype)
...
RSCore.Functions.CreateUseableItem("pistol_ammo", function(source, item)
    ...
    if Player.Functions.RemoveItem("pistol_ammo", 1, item.slot) then
        ...
        TriggerClientEvent('RSCore:Notify', source, RSCore.Shared.Items["pistol_ammo"]["label"].. " gebruikt")
    end
end)
...
RSCore.Functions.CreateUseableItem("rifle_ammo", function(source, item)
    ...
    if Player.Functions.RemoveItem("rifle_ammo", 1, item.slot) then
        ...
        TriggerClientEvent('RSCore:Notify', source, RSCore.Shared.Items["rifle_ammo"]["label"].. " gebruikt")
    end
end)
...
RSCore.Functions.CreateUseableItem("smg_ammo", function(source, item)
    ...
    if Player.Functions.RemoveItem("smg_ammo", 1, item.slot) then
        ...
        TriggerClientEvent('RSCore:Notify', source, RSCore.Shared.Items["smg_ammo"]["label"].. " gebruikt")
    end
end)
...
RSCore.Functions.CreateUseableItem("shotgun_ammo", function(source, item)
    ...
    if Player.Functions.RemoveItem("shotgun_ammo", 1, item.slot) then
        ...
        TriggerClientEvent('RSCore:Notify', source, RSCore.Shared.Items["shotgun_ammo"]["label"].. " gebruikt")
    end
end)
...
RSCore.Functions.CreateUseableItem("mg_ammo", function(source, item)
    ...
    if Player.Functions.RemoveItem("mg_ammo", 1, item.slot) then
        ...
        TriggerClientEvent('RSCore:Notify', source, RSCore.Shared.Items["mg_ammo"]["label"].. " gebruikt")
    end
end)
