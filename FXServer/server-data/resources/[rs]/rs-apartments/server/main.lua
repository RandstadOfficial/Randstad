...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    RSCore.Functions.ExecuteSql(false, "INSERT INTO `apartments` (`name`, `type`, `label`, `citizenid`) VALUES ('"..apartmentId.."', '"..type.."', '"..label.."', '"..Player.PlayerData.citizenid.."')")
    TriggerClientEvent('RSCore:Notify', src, "Je hebt een appartementje ("..label..")")
...
AddEventHandler('apartments:server:UpdateApartment', function(type)
    local src = source
    local Player = RSCore.Functions.GetPlayer(src)
    RSCore.Functions.ExecuteSql(false, "UPDATE `apartments` SET type='"..type.."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'")

    TriggerClientEvent('RSCore:Notify', src, "Je bent van appartement veranderd!")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
function CreateApartmentId(type)
	...
	while not UniqueFound do
		...
		RSCore.Functions.ExecuteSql(true, "SELECT COUNT(*) as count FROM `apartments` WHERE `name` = '"..tostring(type .. AparmentId).."'", function(result)
...
function GetApartmentInfo(apartmentId)
    local retval = nil
    RSCore.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `name` = '"..apartmentId.."' ", function(result)
...
function GetOwnedApartment(citizenid)
    RSCore.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `citizenid` = '"..citizenid.."' ", function(result)
        if result[1] ~= nil then 
            return result[1]
        end
        return nil
...
RSCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
RSCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
RSCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
RSCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
        RSCore.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..cid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
        ...
        local src = source
        local Player = RSCore.Functions.GetPlayer(src)
        RSCore.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
...
RSCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = RSCore.Functions.GetPlayer(src)
    if Player ~= nil then
        RSCore.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
...
RSCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = RSCore.Functions.GetPlayer(src)
...
RegisterServerEvent('rs-apartments:server:SetInsideMeta')
AddEventHandler('rs-apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
    local Player = RSCore.Functions.GetPlayer(src)
