...
RegisterNetEvent('rs-diving:client:NewLocations')
AddEventHandler('rs-diving:client:NewLocations', function()
...
RegisterNetEvent('rs-diving:client:SetDivingLocation')
AddEventHandler('rs-diving:client:SetDivingLocation', function(DivingLocation)
    ...
    Citizen.CreateThread(function()
        RadiusBlip = AddBlipForRadius(RSDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, RSDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, RSDiving.Locations[CurrentDivingLocation.Area].coords.Area.z, 100.0)
        ...
        LabelBlip = AddBlipForCoord(RSDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, RSDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, RSDiving.Locations[CurrentDivingLocation.Area].coords.Area.z)
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentDivingLocation.Area ~= 0 then
            local AreaDistance = GetDistanceBetweenCoords(Pos, RSDiving.Locations[CurrentDivingLocation.Area].coords.Area.x, RSDiving.Locations[CurrentDivingLocation.Area].coords.Area.y, RSDiving.Locations[CurrentDivingLocation.Area].coords.Area.z)
            ...
            if inRange then
                for cur, CoralLocation in pairs(RSDiving.Locations[CurrentDivingLocation.Area].coords.Coral) do
                            ...
                            if not CoralLocation.PickedUp then
                                ...
                                if CoralDistance <= 1.5 then
                                    ...
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        ...
                                        RSCore.Functions.Progressbar("take_coral", "Koraal aan het verzamelen..", times * 1000, false, true, {
...
function TakeCoral(coral)
    RSDiving.Locations[CurrentDivingLocation.Area].coords.Coral[coral].PickedUp = true
    TriggerServerEvent('rs-diving:server:TakeCoral', CurrentDivingLocation.Area, coral, true)
end

RegisterNetEvent('rs-diving:client:UpdateCoral')
AddEventHandler('rs-diving:client:UpdateCoral', function(Area, Coral, Bool)
...
function CallCops()
    ...
    if Call == Chance then
        TriggerServerEvent('rs-diving:server:CallCops', Coords)
    end
end

RegisterNetEvent('rs-diving:server:CallCops')
AddEventHandler('rs-diving:server:CallCops', function(Coords, msg)
    ...
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentString("112 - Duiklocatie")
...
RegisterNetEvent('rs-diving:client:UseGear')
AddEventHandler('rs-diving:client:UseGear', function(bool)
    if bool then
        GearAnim()
        RSCore.Functions.Progressbar("equip_gear", "Duikpak aantrekken..", 5000, false, true, {}, {}, {}, {}, function() -- Done
            DeleteGear()
            ...
            TriggerServerEvent('rs-diving:server:RemoveGear')
        ...
        if currentGear.enabled then
            GearAnim()
            RSCore.Functions.Progressbar("remove_gear", "Duikpak uittrekken..", 5000, false, true, {}, {}, {}, {}, function() -- Done
                DeleteGear()

                SetEnableScuba(GetPlayerPed(-1), false)
                SetPedMaxTimeUnderwater(GetPlayerPed(-1), 1.00)
                currentGear.enabled = false
                TriggerServerEvent('rs-diving:server:GiveBackGear')
                ClearPedTasks(GetPlayerPed(-1))
                RSCore.Functions.Notify('Je hebt je duikpak uitgetrokken')
            end)
        else
            RSCore.Functions.Notify('Je hebt geen duikgear aan..', 'error')
        end
    end
end)
