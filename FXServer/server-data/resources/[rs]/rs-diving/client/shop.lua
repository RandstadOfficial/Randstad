...
Citizen.CreateThread(function()
    while true do
        ...
        if not notInteressted then
            for k, v in pairs(RSDiving.SellLocations) do
                ...
                if dist < 20 then
                    ...
                    if dist < 1 then
                        DrawText3D(v["coords"]["x"], v["coords"]["y"], v["coords"]["z"] - 0.1, '~g~G~w~ - Koraal verkopen')

                        if IsControlJustPressed(0, Keys["G"]) then
                            TriggerServerEvent('rs-diving:server:SellCoral')
                            notInteressted = true
