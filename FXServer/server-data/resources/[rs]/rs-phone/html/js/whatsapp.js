...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    RS.Phone.Functions.SetupChatMessages(ChatData);

    $.post('http://rs-phone/ClearAlerts', JSON.stringify({
...
RS.Phone.Functions.GetLastMessage = function(messages) {
...
RS.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = RS.Phone.Functions.GetLastMessage(chat.messages);
...
RS.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        ...
        RS.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Je kan geen leeg bericht sturen!", "#25D366", 1750);
    }
});
...
// RS.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
//     if (cData) {
//         OpenedChatData.number = cData.number;

//         if (OpenedChatPicture == null) {
//             $.post('http://rs-phone/GetProfilePicture', JSON.stringify({
...
//                 var Sender = "me";
//                 if (message.sender !== RS.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
//                 var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('http://rs-phone/SharedLocation', JSON.stringify({
