...
SetupMeosHome = function() {
    $("#meos-app-name").html("Welkom " + RS.Phone.Data.PlayerData.charinfo.firstname + " " + RS.Phone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.appartment-adress-location', function(e){
    ...
    $.post('http://rs-phone/SetApartmentLocation', JSON.stringify({
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    RS.Phone.Notifications.Add("fas fa-university", "MEOS", "Huis nr. gekopieerd!", "#badc58", 1750);
...
$(document).on('click', '.confirm-search-person-test', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://rs-phone/FetchSearchResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
                ...
                RS.Phone.Notifications.Add("politie", "MEOS", "Er zijn geen zoek resultaten!");
                $(".person-search-results").html("");
            }
        });
    } else {
...
$(document).on('click', '.confirm-search-person-house', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://rs-phone/FetchPlayerHouses', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
...
$(document).on('click', '.confirm-search-vehicle', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://rs-phone/FetchVehicleResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".vehicle-search-results").html("");
...
$(document).on('click', '.scan-search-vehicle', function(e){
    e.preventDefault();
    $.post('http://rs-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            $(".vehicle-search-results").html("");
            var APK = "Ja";
            if (!vehicle.status) {
            ...
            RS.Phone.Notifications.Add("politie", "MEOS", "Geen voertuig in de buurt!");
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData.coords != undefined && alertData.coords != null) {
        ...
        RS.Phone.Notifications.Add("politie", "MEOS", "Deze melding heeft geen GPS Locatie!");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    ...
    RS.Phone.Notifications.Add("politie", "MEOS", "Alle meldingen zijn verwijderd!");
});