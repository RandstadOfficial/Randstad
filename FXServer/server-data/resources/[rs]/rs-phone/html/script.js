...
$(document).on('click', '.payphone-call', function(e){
    ...
    if (CurNumber != "") {
        $.post('http://rs-phone/CallPayPhone', JSON.stringify({
            number: CurNumber
        }));
...
$(document).ready(function(){
    ...
    window.addEventListener('message', function(event){
        ...
        if (eventData.action == "phone") {
            if (!eventData.task) {
                if (eventData.open == true) {
                    elPhone.Open(eventData.cid)
                    elPhone.setupPhoneApps(eventData.apps)
        ...
        if (eventData.task == "callScreen") {
            if (eventData.callData.inCall || eventData.callData.outgoingCall || eventData.callData.incomingCall) {
                elPhone.CallScreen(event.data.callData)
            }
        }

        if (eventData.task == "setPhoneMeta") {
            elPhone.setPhoneMeta(eventData.pMeta, eventData.pData.charinfo.phone);
        }

        if (eventData.task == "setUserContacts") {
            elPhone.setupPlayerContactInfo(eventData.pData.charinfo.firstname, eventData.pData.charinfo.lastname, eventData.pData.charinfo.phone);
            elPhone.setupPlayerContacts(eventData.pContacts);
        }

        if (eventData.task == "setupBankData") {
            elPhone.setBankData(eventData.pData)
        }

        if (eventData.task == "updateTime") {
            elPhone.updateTime(eventData.time)
        }

        if (eventData.task == "getVehicles") {
            elPhone.setGarageVehicles(eventData.vehicles)
        }

        if (eventData.task == "setupMail") {
            elPhone.setupUserMails(eventData.mails)
        }

        if (eventData.task == "setupTweets") {
            elPhone.SetupTweets(eventData.tweets);
        }

        if (eventData.task == "phoneNotify")  {
            elPhone.Notify(eventData.title, eventData.type, eventData.text, 3500);
        }

        if (eventData.task == "setupAds") {
            elPhone.SetupAdverts(eventData.ads);
        }

        if (eventData.task == "phoneNotification") {
            elPhone.phoneNotification(eventData.message)
        }

        if (eventData.task == "newTweet") {
            elPhone.SetupTweets(eventData.tweets);
            elPhone.Notify('<i class="fab fa-twitter" style="position: relative; padding-top: 3px;"></i> Twitter', 'tweet', '@'+eventData.sender + ' heeft een tweet geplaatst!')
        }

        if (eventData.task == "setupCompanies") {
            elPhone.SetupCompanies(eventData.companies);
        }

        if (eventData.task == "newAd") {
            elPhone.SetupAdverts(eventData.ads);
            elPhone.Notify('<i class="fas fa-ad" style="position: relative; padding-top: 3px;"></i> Advertentie', 'advert', eventData.sender + ' heeft een advertentie geplaatst!')
        }

        if (eventData.task == "updateChat") {
            elPhone.UpdateChat(eventData.messages, eventData.number)
        }

        if (eventData.task == "updateChats") {
            $.post('http://rs-phone/getMessages', JSON.stringify({}), function(messages){
                elPhone.SetupChat(messages)
            });
        }

        if (eventData.task == "newMessage") {
            elPhone.Notify('<i class="fas fa-envelope" style="position: relative; padding-top: 7px;"></i> Berichten', 'success', eventData.sender + ' heeft een bericht gestuurd!')
        }

        if (eventData.task == "newPoliceAlert") {
            elPhone.AddPoliceAlert(eventData);
        }
    ...
    $('.notify-btn').change(function() {
        if (allowNotifys == null) {
            if (myNotify) {
                allowNotifys = true;
                elPhone.toggleNotify(true)
            } else {
                allowNotifys = false;
                elPhone.toggleNotify(false)
            }
        } else {
            if (!allowNotifys) {
                allowNotifys = true;
...
$(document).on('click', '#suggestedContact-accept', function(e){
    ...
    $.post('http://rs-phone/setupContacts');
...
$(document).on('click', '#suggestedBank-accept', function(e){
    ...
    $.post('http://rs-phone/getBankData');
...
$(document).on('click', '.app', function(e){
    ...
    if (pressedApp.app == "contacts") {
        ...
        $.post('http://rs-phone/getVehicles');
    } else if (pressedApp.app == "mails") {
        $.post('http://rs-phone/getUserMails');
    } else if (pressedApp.app == "twitter") {
        $.post('http://rs-phone/getTweets');
    } else if (pressedApp.app == "advert") {
        $.post('http://rs-phone/getAds');
    } else if (pressedApp.app == "messages") {
        $.post('http://rs-phone/getMessages', JSON.stringify({}), function(messages){
            elPhone.SetupChat(messages)
        });
    } else if (pressedApp.app == "police") {
        $.post('http://rs-phone/getCharacterData', JSON.stringify({}), function(charinfo){
            elPhone.SetupPoliceApp(charinfo)
        });
    } else if (pressedApp.app == "companies") {
        $.post('http://rs-phone/getCompanies');
    }

    elPhone.succesSound();
});

elPhone.SetupPoliceApp = function(data) {
...
$(document).on('click', '#person-button-search', function(e){
    ...
    $.post('http://rs-phone/policeSearchPerson', JSON.stringify({search: search,}), function(persons){
        if (persons != null) {
...
$(document).on('click', '#vehicle-button-search', function(e){
    ...
    $.post('http://rs-phone/policeSearchVehicle', JSON.stringify({search: search,}), function(vehicles){
        if (vehicles != null) {
...
$(document).on('click', '#vehicle-button-scan', function(e){
    ...
    $.post('http://rs-phone/scanVehiclePlate', JSON.stringify({}), function(vehicleData){
        var status = '<span class="vehicle-status-inactive">Ja</span>'
        if (!vehicleData.status) {
            status = '<span class="vehicle-status-active">Nee</span>'
        }
...
elPhone.AddPoliceAlert = function(data) {
...
$(document).on('click', '.alert-location', function(e){
    ...
    $.post('http://rs-phone/setAlertWaypoint', JSON.stringify({
        alert: alertData,
    }))
});
...
elPhone.SetupChat = function(chats) {
...
$(document).on('click', '.msg-me-location', function(e){
    ...
    $.post('http://rs-phone/setMessageLocation', JSON.stringify({
...
elPhone.UpdateChat = function(messages, number) {
...
$(document).on('click', '.send-message', function(e){
    ...
    if (message != "") {
        $.post('http://rs-phone/sendMessage', JSON.stringify({
...
$(document).on('click', '.sms-contact', function(){
    ...
    $.post('http://rs-phone/doesChatExists', JSON.stringify({
        cData: contactData
    }), function(cbData){
        if (cbData != "") {
            elPhone.OpenExistingChat(cbData)
        } else {
            elPhone.OpenNewChat(contactData)
        }
    });
...
elPhone.OpenExistingChat = function(messageData) {
    $('.chat-window').css({"display":"block"}).animate({top: "3%",}, 250);
...
elPhone.OpenNewChat = function(data) {
    $('.chat-window').css({"display":"block"}).animate({top: "3%",}, 250);
...
$(document).on('click', '.submit-contact-btn', function(e){
    ...
    if(contactName != "" || contactNum != "") {
        if (!isNaN(contactNum)) {
            ...
            elPhone.Notify('Contacten', 'success', contactName+" ("+contactNum+") is toegevoegd aan je contacten.")
            $.post('http://rs-phone/addToContact', JSON.stringify({
                contactName: contactName,
                contactNum: contactNum
            }))
        } else {
            elPhone.Notify('Contacten', 'error', 'Het telefoon nummer moet bestaan uit cijfers.')
        }
    } else {
        elPhone.Notify('Contacten', 'error', 'Je hebt niet alle contactgegevens ingevuld.')
    }
});
...
$(document).on('click', '.submit-quit-btn', function(e){
    var companyData = $(this).data("companyData");
    $.post('http://rs-phone/removeCompany', JSON.stringify({
...
$(document).on('click', '.submit-quitperson-btn', function(e){
    var companyData = $(this).data("companyData");
    $.post('http://rs-phone/quitCompany', JSON.stringify({
        name: companyData.name,
    }))
    ...
    if (ibanVal != "" && amountVal != "") {
        if (!isNaN(amountVal)) {
            if (balance - amountVal < 0) {
                elPhone.Notify('EBank', 'error', 'Je hebt niet genoeg saldo', 3500);
            } else {
                if (amountVal > 0) {
                    $('.transfer-money-container').css({"display":"block"}).animate({top: "103%",}, 250, function(){
                        $('.transfer-money-container').css({'display':'none'});
                        elPhone.Notify('EBank', 'success', 'Je hebt € '+amountVal+' overgemaakt naar '+ibanVal+'!', 3500);
    
                        $.post('http://rs-phone/transferMoney', JSON.stringify({
                            amount: amountVal,
                            iban: ibanVal
                        }));
                    ...
                    elPhone.Notify('EBank', 'error', 'Hoeveelheid mag niet lager zijn dan 0', 3500);
                }
            }
        } else {
            elPhone.Notify('EBank', 'error', 'De hoeveelheid moet bestaan uit cijfers.', 3500)
        }
    } else {
        elPhone.Notify('EBank', 'error', 'Je hebt niet alle gegevens ingevuld.', 3500)
    }
});
...
$(document).on('click', '.background-option', function(e){
    ...
    $.post('http://rs-phone/setPlayersBackground', JSON.stringify({
...
elPhone.Close = function() {
    $(".payphone").fadeOut(150);
    $.post('http://rs-phone/closePayPhone');
    inPhone = false;
    $('.phone-container').css({'display':'block'}).animate({
        top: "100%",
    }, 300);
...
elPhone.setupPhoneApps = function(apps) {
    $('.app-slot').html("");
...
elPhone.Log = function(log) {
    console.log(log)
}

elPhone.toggleNotify = function(bool) {
    allowNotifys = bool;
    if (bool) {
    ...
    $.post('http://rs-phone/setNotifications', JSON.stringify({allow: bool}));
    elPhone.succesSound();
}

elPhone.succesSound = function() {
    $.post('http://rs-phone/succesSound');
}

elPhone.errorSound = function() {
    $.post('http://rs-phone/errorSound');
}

elPhone.setPhoneMeta = function(phoneMeta, phoneNumber) {
    elPhone.setPlayersNotification(phoneMeta.settings.notification)
    elPhone.setPlayersBackground(phoneMeta.settings.background)
    elPhone.setPlayersPhoneNumber(phoneNumber)
}

elPhone.setPlayersNotification = function(allow) {
    myNotify = allow;
    if (allow) {
...
elPhone.setPlayersPhoneNumber = function(num) {
    $('.phone-num').html(num);
}

elPhone.setPlayersBackground = function(background) {
...
elPhone.updateTime = function(time) {
    $("#time").html(time.hour+"."+time.minute);
}

elPhone.setupPlayerContactInfo = function(firstName, lastName, phoneNumber) {
...
$(document).on('click', '.change-contact-btn', function(e){
    ...
    if($(".edit-number-input").val() != "" || $(".edit-contactname-input").val() != "") {
        if (!isNaN($(".edit-number-input").val())) {
            ...
            elPhone.Notify('Contacten', 'success', 'Contact opgeslagen');
            $('.edit-contact-container').css({"display":"block"}).animate({top: "103%",}, 250, function(){
                $(".edit-contact-container").css({"display":"none"});
            });
        } else {
...
$(document).on('click', '.delete-contact-btn', function(e){
    ...
    $.post('http://rs-phone/removeContact', JSON.stringify({
        oldContactName: oldContactName,
        oldContactNum: oldContactNum,
    }))
...
elPhone.setupPlayerContacts = function(contacts) {
    $(".contact-list").html("");
...
elPhone.Notify = function(title, type, message, wait) {
    // if (allowNotifys) {
...
elPhone.setBankData = function(playerData) {
...
elPhone.setGarageVehicles = function(vehicles) {
...
$(document).on('click', '.mail', function(){
    ...
    elPhone.openMail(mailData)
});

$(document).on('click', '.mail-button', function(){
    $.post('http://rs-phone/clickMailButton', JSON.stringify({
...
elPhone.openMail = function(mailData) {
    ...
    if (mailData.read == 0) {
        $.post('http://rs-phone/setEmailRead', JSON.stringify({
...
$(document).on('click', '.send-new-tweet', function(){
    ...
    if (tweetMessage != "") {
        ...
        $.post('http://rs-phone/postTweet', JSON.stringify({
            message: tweetMessage
        }))
    } else {
        elPhone.Notify('Twitter', 'error', 'Je kan geen legen tweet versturen...')
    }
});
...
$(document).on('click', '.send-new-ad', function(){
    ...
    if (advertMessage != "") {
        ...
        elPhone.Notify('Advertenties', 'error', 'Je kan geen legen tweet versturen...')
    }
});
...
elPhone.phoneNotification = function(message) {
...
elPhone.SetupTweets = function(tweets) {
    $('.twitter-tweets').html("");
    if (tweets != undefined) {
...
elPhone.SetupCompanies = function(companies) {
...
elPhone.SetupAdverts = function(ads) {
    $('.advert-ads').html("");
    if (ads != undefined) {
...
elPhone.CallScreen = function(callData) {
