...
RegisterNetEvent('rs-phone:client:CallPayPhone')
AddEventHandler('rs-phone:client:CallPayPhone', function(num)
    TriggerServerEvent('rs-phone:server:PayPayPhone', 20, num)
end)

RegisterNetEvent('rs-phone:client:CallPayPhoneYes')
AddEventHandler('rs-phone:client:CallPayPhoneYes', function(num)
    if inPayPhoneRange then
        ...
        RSCore.Functions.Notify('Oproep gestart met '..callData.name, 'primary', 2500)
        ...
        Citizen.CreateThread(function()
            for i = 1, 10, 1 do
                if callData.outgoingCall then
                    ...
                    RSCore.Functions.Notify('Oproep is bezig, /ophangen om gesprek te stoppen', 'primary', 2000)
                    callTime = i
                    if callTime == 10 then
                        Citizen.Wait(3000)
                        RSCore.Functions.Notify('Er word niet opgenomen..', 'error', 3500)
                        callData.number = nil
                        callData.name = nil
                        callData.callId = 0
                        callData.inCall = false
        ...
        TriggerServerEvent('rs-phone:server:CallContact', callData, "06"..math.random(11111111, 99999999), true)
    else
        RSCore.Functions.Notify('Je bent niet bij een telefooncel in de buurt', 'error', 2500)
    end
end)

RegisterNUICallback('CallPayPhone', function(data)
    TriggerEvent('rs-phone:client:CallPayPhone', tostring(data.number))
end)
...
RegisterNUICallback('getCharacterData', function(data, cb)
    cb(RSCore.Functions.GetPlayerData().charinfo)
end)

RegisterNUICallback('policeSearchPerson', function(data, cb)
    RSCore.Functions.TriggerCallback('rs-phone:server:getSearchData', function(result)
        cb(result)
    end, data.search)
end)

RegisterNUICallback('policeSearchVehicle', function(data, cb)
    RSCore.Functions.TriggerCallback('rs-phone:server:getVehicleSearch', function(result)
        if result ~= nil then 
...
RegisterNUICallback('scanVehiclePlate', function(data, cb)
    ...
    RSCore.Functions.TriggerCallback('rs-phone:server:getVehicleData', function(result)
        RSCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
            local vehicleInfo = RSCore.Shared.VehicleModels[model] ~= nil and RSCore.Shared.VehicleModels[model] or {["brand"] = "Onbekend merk..", ["name"] = ""}
...
RegisterNetEvent('rs-phone:client:giveNumber')
AddEventHandler('rs-phone:client:giveNumber', function(data)
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        TriggerServerEvent('rs-phone:server:giveNumber', playerId, PlayerData)
    else
        RSCore.Functions.Notify("Niemand in de buurt!", "error")
    end
end)

RegisterNetEvent('rs-phone:client:giveBankAccount')
AddEventHandler('rs-phone:client:giveBankAccount', function(data)
    local ped = GetPlayerPed(-1)
    local PlayerData = RSCore.Functions.GetPlayerData()
    ...
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent('rs-phone:server:giveBankAccount', playerId, PlayerData)
    else
        RSCore.Functions.Notify("Niemand in de buurt!", "error")
    end
end)

RegisterNetEvent('rs-phone:server:newContactNotify')
AddEventHandler('rs-phone:server:newContactNotify', function(number)
    RSCore.Functions.Notify('[M] Je hebt een nieuw voorgesteld contactpersoon!')
    SendNUIMessage({
...
RegisterNetEvent('rs-phone:server:newBankNotify')
AddEventHandler('rs-phone:server:newBankNotify', function(nr)
    RSCore.Functions.Notify('[M] Je hebt een nieuw voorgesteld bankrekening nr.!')
    SendNUIMessage({
...
RegisterNUICallback('doesChatExists', function(data, cb)
    RSCore.Functions.TriggerCallback('rs-phone:server:doesChatExists', function(result)
        local cbData = {}
        if result ~= nil then
            contactName = nil
            cbData = {
    ...
    if exist then
        if type == "normal" then
            table.insert(messages[cur].messages, {
                sender = RSCore.Functions.GetPlayerData().citizenid,
        ...
        TriggerServerEvent('rs-phone:server:sendMessage', messages[cur])
        RSCore.Functions.Notify('Bericht verstuurd!', 'success', 2500)
    else
        if type == "normal" then
        ...
        TriggerServerEvent('rs-phone:server:createChat', messages[cur])
    end
...
RegisterNUICallback('setMessageLocation', function(data)
    ...
    RSCore.Functions.Notify('GPS Locatie ingesteld!', 'success')
...
RegisterNetEvent('rs-phone:client:createChatOther')
AddEventHandler('rs-phone:client:createChatOther', function(chatData, senderPhone)
    table.insert(messages, {
        number = senderPhone,
        name = contactName,
    ...
    TriggerServerEvent('rs-phone:server:createChatOther', chatData, senderPhone)
end)

RegisterNetEvent('rs-phone:client:recieveMessage')
AddEventHandler('rs-phone:client:recieveMessage', function(chatData, senderPhone)
    for _, chat in pairs(messages) do
        if chat.number == senderPhone then
            chat.messages = chatData.messages
        end
    end
    TriggerServerEvent('rs-phone:server:recieveMessage', chatData, senderPhone)
    SendNUIMessage({
        task = "updateChat",
        messages = chatData.messages,
        number = senderPhone
...
RegisterNetEvent('rs-phone:client:msgNotify')
AddEventHandler('rs-phone:client:msgNotify', function(msg, contact)
    if not inPhone then
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            RSCore.Functions.TriggerCallback('rs-phone:server:getUserContacts', function(result)
...
AddEventHandler('RSCore:Client:OnPlayerLoaded', function()
    ...
    RSCore.Functions.TriggerCallback('rs-phone:server:getPlayerMessages', function(result)
        messages = result
    end)
end)
...
function setPhoneMeta()
    phoneMeta = RSCore.Functions.GetPlayerData().metadata["phone"]

    if next(phoneMeta) == nil then
        phoneMeta = defaultPhoneMeta
        TriggerServerEvent('rs-phone:server:setPhoneMeta', defaultPhoneMeta)
    end

    SendNUIMessage({
        task = "setPhoneMeta",
        pMeta = phoneMeta,
        pData = RSCore.Functions.GetPlayerData(),
    })
    
    TriggerServerEvent('rs-phone:server:getContacts')
end
...
RegisterNUICallback('getVehicles', function()
    RSCore.Functions.TriggerCallback('rs-phone:server:GetAllUserVehicles', function(vehicles)
        local myVehicles = {}
        ...
        for k, v in pairs(vehicles) do
            ...
            table.insert(myVehicles, {name = RSCore.Shared.Vehicles[vehicles[k].vehicle]["name"], plate = vehicles[k].plate, garage = garage, state = state, engine = vehicles[k].engine, body = vehicles[k].body, fuel = vehicles[k].fuel, image = RSCore.Shared.Vehicles[vehicles[k].vehicle]["image"]})
        end
...
RegisterNUICallback('addToContact', function(data)
    ...
    RSCore.Functions.TriggerCallback('rs-phone:server:getContactStatus', function(stat)
    ...
    TriggerServerEvent('rs-phone:server:addContact', contactName, contactNum)
    RSCore.Functions.Notify(contactNum..' is toegevoegd aan je contacten!', 'success', 3500)
end)
...
RegisterNUICallback('editContact', function(data)
    ...
    TriggerServerEvent('rs-phone:server:editContact', oldContactName, oldContactNum, newContactName, newContactNum)
    RSCore.Functions.Notify(oldContactNum..' is aangepast!', 'success', 3500)
end)
...
RegisterNUICallback('removeContact', function(data)
    ...
    TriggerServerEvent('rs-phone:server:removeContact', cName, cNumb)
    RSCore.Functions.Notify(cName..' is verwijderd!', 'success', 3500)
...
RegisterNUICallback('transferMoney', function(data)
    if data.amount > 0 then
        TriggerServerEvent('rs-phone:server:transferBank', data.amount, data.iban)
    else
        RSCore.Functions.Notify('Bedrag moet hoger zijn dan 0', 'error')
    end
end)

RegisterNUICallback('getUserMails', function()
    RSCore.Functions.TriggerCallback('rs-phone:server:GetUserMails', function(mails)
        SendNUIMessage({
            task = "setupMail",
            mails = mails
        })
...
RegisterNUICallback('setEmailRead', function(data)
    TriggerServerEvent('rs-phone:server:setEmailRead', data.mailId)
...
RegisterNetEvent('rs-phone:client:RecievedBankNotify')
AddEventHandler('rs-phone:client:RecievedBankNotify', function(amount, iban)
    RSCore.Functions.TriggerCallback('RSCore:HasItem', function(result)
        if result then
            SendNUIMessage({
                task = "phoneNotification",
...
RegisterNUICallback('removeMail', function(data)
    TriggerServerEvent('rs-phone:server:removeMail', data.mailId)
end)

RegisterNUICallback('getTweets', function()
    RSCore.Functions.TriggerCallback('rs-phone:server:getPhoneTweets', function(tweets)
        SendNUIMessage({
            task = "setupTweets",
            tweets = tweets
        })
...
RegisterNetEvent('rs-phone:client:InPhoneNotify')
AddEventHandler('rs-phone:client:InPhoneNotify', function(title, type, text)
    SendNUIMessage({
        task = "phoneNotify",
        title = title,
        type = type, 
...
RegisterNetEvent('rs-phone:client:setupCompanies')
AddEventHandler('rs-phone:client:setupCompanies', function()
    SendNUIMessage({
        task = "setupCompanies",
        companies = exports['el-companies']:GetCompanies()
    })
end)
...
RegisterNUICallback('removeCompany', function(data)
    TriggerServerEvent("el-companies:server:removeCompany", data.name)
end)

RegisterNUICallback('quitCompany', function(data)
    TriggerServerEvent("el-companies:server:quitCompany", data.name)
end)

RegisterNUICallback('getAds', function()
    RSCore.Functions.TriggerCallback('rs-phone:server:getPhoneAds', function(ads)
        SendNUIMessage({
            task = "setupAds",
            ads = ads
        })
...
RegisterNUICallback('postTweet', function(data)
    TriggerServerEvent('rs-phone:server:postTweet', data.message)
end)

RegisterNUICallback('postAdvert', function(data)
    TriggerServerEvent('rs-phone:server:postAdvert', data.message)
end)

RegisterNetEvent('rs-phone:client:newTweet')
AddEventHandler('rs-phone:client:newTweet', function(sender, message)
    RSCore.Functions.TriggerCallback('RSCore:HasItem', function(result)
        if result then
            if inPhone then
                RSCore.Functions.TriggerCallback('rs-phone:server:getPhoneTweets', function(tweets)
...
RegisterNetEvent('rs-phone:client:newAd')
AddEventHandler('rs-phone:client:newAd', function(sender, message)
    RSCore.Functions.TriggerCallback('RSCore:HasItem', function(result)
        if result then
            if inPhone then
                RSCore.Functions.TriggerCallback('rs-phone:server:getPhoneAds', function(ads)
...
function openPhone(bool)
    ...
    if bool then
        SendNUIMessage({
            ...
            open = bool,
            apps = GetPhoneApps(),
            cid = RSCore.Functions.GetPlayerData().citizenid
        })
...
function GetPhoneApps()
    ...
    for k, v in pairs(Config.PhoneApps) do
        if Config.PhoneApps[k].job == nil then 
            apps[k] = v
        elseif (Config.PhoneApps[k].job == RSCore.Functions.GetPlayerData().job.name) and RSCore.Functions.GetPlayerData().job.onduty then
            apps[k] = v
        end
    end
    return apps
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(0, Keys["M"]) then
            local isHandcuffed = RSCore.Functions.GetPlayerData().metadata["ishandcuffed"]
...
RegisterNUICallback('setNotifications', function(data)
    ...
    if allow then
        RSCore.Functions.Notify('Telefoon notificaties zijn ingeschakeld...', 'primary', 3500) 
    else
        RSCore.Functions.Notify('Telefoon notificaties zijn uitgeschakeld...', 'primary', 3500)
    end
...
RegisterNUICallback('closePhone', function()
    openPhone(false)
    inPhone = false
    TriggerServerEvent('rs-phone:server:setPhoneMeta', phoneMeta)
end)
...
RegisterNUICallback('CallContact', function(data)
    ...
    TriggerServerEvent('rs-phone:server:CallContact', callData, pData.charinfo.phone)
end)

RegisterNetEvent('rs-phone:client:CallNumber')
AddEventHandler('rs-phone:client:CallNumber', function(number)
    RSCore.Functions.TriggerCallback('RSCore:HasItem', function(result)
        if result then
            if not callData.outgoingCall or not callData.inComingCall or not callData.inCall then
                ...
                RSCore.Functions.Notify('Oproep gestart met '..callData.number, 'primary', 2500)
...
RegisterNetEvent('rs-phone:client:IncomingCall')
AddEventHandler('rs-phone:client:IncomingCall', function(cData, caller)
    RSCore.Functions.TriggerCallback('RSCore:HasItem', function(result)
        if result then
...
function inComingCall()
    RSCore.Functions.TriggerCallback('rs-phone:server:getContactName', function(name)
        callData.name = name
        ...
        Citizen.CreateThread(function()
            for i = 1, 10, 1 do
                if callData.incomingCall then
                    TriggerServerEvent("InteractSound_SV:PlayOnSource", "demo", 0.1)
                    RSCore.Functions.Notify('Inkomend gesprek van '..number..', /opnemen of /ophangen', 'primary', 2000)
                else
                    break
                end
                Citizen.Wait(3000)
...
RegisterNUICallback('AnswerCall', function()
    TriggerEvent('rs-phone:client:AnswerCall')
end)

RegisterNUICallback('DenyCall', function()
    TriggerEvent('rs-phone:client:HangupCall')
end)
        ...
        if result then
            if callData.incomingCall then
                if not callData.inCall then
                    ...
                    TriggerServerEvent('rs-phone:server:AnswerCall', callData)
                    ...
                    RSCore.Functions.Notify('Je bent al in gesprek..', 'error')
                end
            else
                RSCore.Functions.Notify('Je hebt geen inkomend oproep..', 'error')
            end
        end
    end, "phone")
end)

RegisterNetEvent('rs-phone:client:AnswerCallOther')
AddEventHandler('rs-phone:client:AnswerCallOther', function()
    callData.inCall = true
    callData.outgoingCall = false
...
RegisterNetEvent('rs-phone:client:HangupCallOther')
AddEventHandler('rs-phone:client:HangupCallOther', function(cData)
    if cData.callId == callData.callId then
        exports.tokovoip_script:removePlayerFromRadio(callData.callId)
        RSCore.Functions.Notify('Het gesprek is beëindigd')
...
RegisterNetEvent('rs-phone:client:HangupCall')
AddEventHandler('rs-phone:client:HangupCall', function()
    RSCore.Functions.TriggerCallback('RSCore:HasItem', function(result)
        if result then
            if callData.inCall then
                exports.tokovoip_script:removePlayerFromRadio(callData.callId)
            else
                RSCore.Functions.Notify('Je zit niet in een gesprek..', 'error')
            end

            TriggerServerEvent('rs-phone:server:HangupCall', callData)
            RSCore.Functions.Notify('Het gesprek is beëindigd')
