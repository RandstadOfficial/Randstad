...
Citizen.CreateThread(function()
	while true do
		...
		for k,doorID in ipairs(RS.Doors) do
			local distance
...
AddEventHandler('lockpicks:UseLockpick', function()
	...
	RSCore.Functions.TriggerCallback('rs-radio:server:GetItem', function(hasItem)
		for k, v in pairs(RS.Doors) do
			local dist = GetDistanceBetweenCoords(pos, RS.Doors[k].textCoords.x, RS.Doors[k].textCoords.y, RS.Doors[k].textCoords.z)
			if dist < 1.5 then
				if RS.Doors[k].pickable then
					if RS.Doors[k].locked then
						if hasItem then
							closestDoorKey, closestDoorValue = k, v
							TriggerEvent('rs-lockpick:client:openLockpick', lockpickFinish)
						else
							RSCore.Functions.Notify("Je mist een toolkit..", "error")
						end
					else
						RSCore.Functions.Notify('De deur is al ontgrendeld??', 'error', 2500)
					end
				else
					RSCore.Functions.Notify('De deur heeft een te sterk slot', 'error', 2500)
				end
			end
		end
    end, "screwdriverset")
...
function setDoorLocking(doorId, key)
    ...
    SetTimeout(400, function()
		...
		TriggerServerEvent('rs-doorlock:server:updateState', key, doorId.locked)
	end)
end
...
RegisterNetEvent('rs-doorlock:client:setState')
AddEventHandler('rs-doorlock:client:setState', function(doorID, state)
	RS.Doors[doorID].locked = state
end)

RegisterNetEvent('rs-doorlock:client:setDoors')
AddEventHandler('rs-doorlock:client:setDoors', function(doorList)
	RS.Doors = doorList
end)

RegisterNetEvent('RSCore:Client:OnPlayerLoaded')
AddEventHandler('RSCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("rs-doorlock:server:setupDoors")
end)