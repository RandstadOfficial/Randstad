...
mainMenu = NativeUI.CreateMenu("Admin Menu", "Randstad Admin", 1400, 100)
    ...
    submenu.OnCheckboxChange = function(sender, item, checked_)
        if item == joinItem then
            ...
            if isClosed then
                ...
                TriggerServerEvent("RSCore:server:CloseServer", reason)
            else
                TriggerServerEvent("RSCore:server:OpenServer")
            end
        end
    end
end
...
RegisterNetEvent("rsadmin:client:SetServerStatus")
AddEventHandler("rsadmin:client:SetServerStatus", function(isClosed)
