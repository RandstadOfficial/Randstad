...
function CheckForVehicles()
    ...
    local veh = 0
    for k, v in pairs(Validvehicles) do
...
AddEventHandler('hospital:client:TakeBrancard', function()
    ...
    if Vehicle ~= 0 then
        ...
        RSCore.Functions.Notify("Je staat niet bij een Ambulance voertuig..", 'error')
    end
end)
    ...
    if BrancardObject ~= nil then
        ...
        if Dist < 3.0 then
            if DoesEntityExist(BrancardObject) then
                ...
                TriggerServerEvent('rs-radialmenu:server:RemoveBrancard', PlayerPos, BrancardObject)
                IsAttached = false
                BrancardObject = nil
                IsLayingOnBed = false
            end
        else
            RSCore.Functions.Notify('Je staat te ver weg!', 'error')
        end
    end
end)
        ...
        if BrancardObject ~= nil then
            ...
            if Distance <= 1.5 then
                if not IsAttached then
                    ...
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~g~E~w~ - Brancard loslaten')
                    if IsControlJustPressed(0, 51) then
                        DetachBrancard()
                        IsAttached = false
                    end
                ...
                if not IsLayingOnBed then
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, '~g~G~w~ - Op brancard liggen')
            ...
            elseif Distance <= 2 then
                if not IsLayingOnBed then
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, 'Ga hier staan')
                else
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, '~g~G~w~ - Afstappen')
                        if IsControlJustPressed(0, Keys["G"]) or IsDisabledControlJustPressed(0, Keys["G"]) then
                            GetOffBrancard()
                        end
                    end
...
RegisterNetEvent('rs-radialmenu:client:RemoveBrancardFromArea')
AddEventHandler('rs-radialmenu:client:RemoveBrancardFromArea', function(PlayerPos, BObject)
    local Ped = GetPlayerPed(-1)
...
function LayOnBrancard()
    ...
    if player == -1 then
        ...
        if distance < 2.0 then
            TriggerServerEvent('rs-radialmenu:Brancard:BusyCheck', GetPlayerServerId(player), "lay")
        else
            LoadAnim(inBedDicts)
            if Object ~= nil or Object ~= 0 then
                TaskPlayAnim(PlayerPedId(), inBedDicts, inBedAnims, 8.0, 8.0, -1, 69, 1, false, false, false)
...
RegisterNetEvent('rs-radialmenu:Brancard:client:BusyCheck')
AddEventHandler('rs-radialmenu:Brancard:client:BusyCheck', function(OtherId, type)
    local ped = GetPlayerPed(-1)
    if type == "lay" then
        ...
        if IsEntityPlayingAnim(ped, "anim@gangops@morgue@table@", "ko_front", 3) then
            TriggerServerEvent('rs-radialmenu:server:BusyResult', true, OtherId, type)
        else
            TriggerServerEvent('rs-radialmenu:server:BusyResult', false, OtherId, type)
        end
    else
...
RegisterNetEvent('rs-radialmenu:client:Result')
AddEventHandler('rs-radialmenu:client:Result', function(IsBusy, type)
    local inBedDicts = "anim@gangops@morgue@table@"
    local inBedAnims = "ko_front"
    local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            RSCore.Functions.Notify("Deze brancard is al in gebruik!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
...
function AttachToBrancard()
    ...
    if BrancardObject ~= nil then
        if ClosestPlayer == -1 then
            ...
            if distance < 2.0 then
                TriggerServerEvent('rs-radialmenu:Brancard:BusyCheck', GetPlayerServerId(ClosestPlayer), "attach")
            else
                NetworkRequestControlOfEntity(BrancardObject)
                LoadAnim("anim@heists@box_carry@")
                TaskPlayAnim(PlayerPed, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)
