                ...
                if seat == ped then
                    if not CheckDone then
                        if vehiclemeters == -1 then
                            CheckDone = true
                            RSCore.Functions.TriggerCallback('rs-vehicletuning:server:IsVehicleOwned', function(IsOwned)
                                if IsOwned then
...
RegisterNetEvent('rs-vehicletuning:client:UpdateDrivingDistance')
AddEventHandler('rs-vehicletuning:client:UpdateDrivingDistance', function(amount, plate)
