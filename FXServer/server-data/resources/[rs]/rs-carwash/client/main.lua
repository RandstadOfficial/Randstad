                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('rs-carwash:server:washCar')
                                end
                            else
...
RegisterNetEvent('rs-carwash:client:washCar')
AddEventHandler('rs-carwash:client:washCar', function()
    local PlayerPed = GetPlayerPed(-1)
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    local Driver = GetPedInVehicleSeat(PedVehicle, -1)
    ...
    RSCore.Functions.Progressbar("search_cabin", "Voertuig wordt gewassen..", math.random(4000, 8000), false, true, {
        ...
        RSCore.Functions.Notify("Wassen geannuleerd..", "error")
        washingVehicle = false
    end)
end)
