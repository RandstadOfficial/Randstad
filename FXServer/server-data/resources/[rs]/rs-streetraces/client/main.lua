                    ...
                    if Races[k] ~= nil then
                        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[k].startx, Races[k].starty, Races[k].startz, true) < 15.0 and not Races[k].started then
                            ...
                            if IsControlJustReleased(0, Keys["H"]) then
                                TriggerServerEvent("rs-streetraces:JoinRace", k)
                            end
                        end
                    end
            ...
            if RaceId ~= 0 and InRace then
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 250.0 and Races[RaceId].started then
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 15.0 then
                        TriggerServerEvent("rs-streetraces:RaceWon", RaceId)
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('rs-streetraces:StartRace')
AddEventHandler('rs-streetraces:StartRace', function(race)
...
RegisterNetEvent('rs-streetraces:RaceDone')
AddEventHandler('rs-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        RSCore.Functions.Notify("Race afgelopen! De winnaar is "..winner.. "!")
    end
end)

RegisterNetEvent('rs-streetraces:StopRace')
AddEventHandler('rs-streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)

RegisterNetEvent('rs-streetraces:CreateRace')
AddEventHandler('rs-streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(GetPlayerPed(-1), true)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            TriggerServerEvent("rs-streetraces:NewRace", race)
            RSCore.Functions.Notify("Race gemaakt voor €"..amount..",-!", "success")
        else
            RSCore.Functions.Notify("Eind positie te dichtbij!", "error")
        end
    else
        RSCore.Functions.Notify("Je moet een marker neerzetten!", "error")
    end
end)

RegisterNetEvent('rs-streetraces:SetRace')
AddEventHandler('rs-streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('rs-streetraces:SetRaceId')
AddEventHandler('rs-streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        RSCore.Functions.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    RSCore.Functions.Notify("GOOOOOOOOO!!!")
end
