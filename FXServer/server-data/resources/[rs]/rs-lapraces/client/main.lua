                ...
                if Races[CurrentRace].started then 
                    if Races[CurrentRace].checkpoints[CurrentCheckpoint+1] ~= nil then 
                        ...
                        if CurrentLap >= Races[CurrentRace].laps then 
                            TriggerServerEvent("rs-lapraces:server:RaceWinner", CurrentRace)
                        end
                    end
                end
            else
                    ...
                    if Races[ClosestRace].active and not Races[ClosestRace].started then 
                        ...
                        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[ClosestRace].checkpoints[1].x, Races[ClosestRace].checkpoints[1].y, Races[ClosestRace].checkpoints[1].z, true) < 10.0 then
                            ...
                            if IsControlJustReleased(0, Keys["H"]) then
                                TriggerServerEvent("rs-lapraces:server:JoinRace", ClosestRace)
                            end
                        end
                    end
                end
...
RegisterNetEvent('rs-lapraces:client:RaceCreated')
AddEventHandler('rs-lapraces:client:RaceCreated', function(raceId, raceInfo)
    Races[raceId] = raceInfo
end)

RegisterNetEvent('rs-lapraces:client:RaceJoined')
AddEventHandler('rs-lapraces:client:RaceJoined', function(raceId)
    CurrentRace = raceId
    ClosestRace = nil
    ...
    RSCore.Functions.Notify("Joined race: " .. Races[CurrentRace].label)
end)

RegisterNetEvent('rs-lapraces:client:StartRace')
AddEventHandler('rs-lapraces:client:StartRace', function(raceId)
    if Races[raceId] ~= nil then
        ...
        if CurrentRace == raceId then
            ...
            while countDown > 0 do
                ...
                RSCore.Functions.Notify(countDown, 'primary', 800)
                countDown = countDown - 1
                Citizen.Wait(1000)
            end
            FreezeEntityPosition(GetVehiclePedIsIn(GetPlayerPed(-1), true), false)
            RSCore.Functions.Notify("GOOOOOOOO!")
        end
    else
        RSCore.Functions.Notify("Race doesn't exist???", "error")
    end
end)

RegisterNetEvent('rs-lapraces:client:RaceFinished')
AddEventHandler('rs-lapraces:client:RaceFinished', function(raceId, winner)
    if Races[raceId] ~= nil then
        if CurrentRace == raceId then
            RSCore.Functions.Notify("De race is afgelopen! De winnaar is: " .. winner)
            TriggerEvent("rs-lapraces:client:RaceLeft")
        end
    end
end)

RegisterNetEvent('rs-lapraces:client:RaceStopped')
AddEventHandler('rs-lapraces:client:RaceStopped', function(raceId)
    if Races[raceId] ~= nil then
        if CurrentRace == raceId then
            RSCore.Functions.Notify("De race is stopgezet!", "error")
            TriggerServerEvent("rs-lapraces:client:Refund", raceId)
            TriggerEvent("rs-lapraces:client:RaceLeft")
        end
    end
end)
...
AddEventHandler('rs-lapraces:client:LeaveRace', function()
    if Races[raceId] ~= nil then
        if CurrentRace == raceId then
            RSCore.Functions.Notify("De race is stopgezet!", "error")
            TriggerServerEvent("rs-lapraces:client:Refund", raceId)
...
RegisterNetEvent('rs-lapraces:client:RaceLeft')
AddEventHandler('rs-lapraces:client:RaceLeft', function()
    for k, v in pairs(Races[CurrentRace].checkpoints) do
        if DoesBlipExist(Races[CurrentRace].checkpoints[k].blip) then
            RemoveBlip(Races[CurrentRace].checkpoints[k].blip)
        end
    end
    RSCore.Functions.Notify("Left race: " .. Races[CurrentRace].label, "error")
    CurrentRace = nil
end)
