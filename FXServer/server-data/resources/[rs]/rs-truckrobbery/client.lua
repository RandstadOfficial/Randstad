...
function PlateCheck(CurPlate, CurrentPlate)
    if CurrentPlate ~= CurPlate then
        ...
        exports['pogressBar']:drawBar(3500, 'You are opening the door')
        Citizen.Wait(3500)
        looting = true
        isRobbing = true
        TriggerEvent('animation:rob')
        exports['pogressBar']:drawBar(Config.Timer * 1000, 'Taking Cash/Items')
...
RegisterNetEvent('rs-truckrobbery:rob')
AddEventHandler('rs-truckrobbery:rob', function()
    local pos = GetEntityCoords(GetPlayerPed(-1))
    local stockade = GetHashKey('stockade')
    local vehicle = GetClosestVehicle(pos.x, pos.y, pos.z, 5.001, stockade, 70)
    local text = GetOffsetFromEntityInWorldCoords(vehicle, 0.0, -4.25, 0.0)
            ...
            if dstCheck < 5.0 then
                if not hasRobbed then
                    if not RobbedPlates[Plate] then
                        if CurrentCops >= Config.MinimumTruckPolice then
                            CurPlate = GetVehicleNumberPlateText(vehicle)
                            
                            RSCore.Functions.Progressbar("deur_openen", "You are opening the door..", 3500, false, true, {
                                ...
                                StopAnimTask(GetPlayerPed(-1), "mp_common", "givetake1_a", 1.0)
                        ...
                        RSCore.Functions.Notify("Deze truck lijkt leeg te zijn (al berooft)", "error")
                    end
                end
            end
        end
...
function cb1(success, timeremaining)
    if success then
        ...
        RSCore.Functions.Notify("Je hebt niet kunnen hacken, je moet 30 seconden wachten", "error")
        TriggerEvent('mhacking:hide')
        TriggerServerEvent('RS7x:NotifyPolice', street1, street2, pos)
        Hacking = false
        Wait(30 * 1000) -- add a time penalty if failed, so it gives police more time to arrive // feel free to remove
        isRobbing = false
        RSCore.Functions.Notify("Je kunt de geldtransport weer overvallen!", "success")
        TriggerServerEvent('RS7x:moneytruck_false')
    end
end
