Config = {}

Config.MinimumTruckPolice = 0

Config.Item = "security_card_03" -- item needed to start robbing

Config.Items = { -- Reward items
	"goldbar",
}

Config.Timer = 100 -- how long you want to rob for (progressbar / payout time)

Config.Timeout = 60 -- Timeout before robbing another truck (5 Min Seconds)

Config.BlipTimer = 30 -- Blip timer  until removed (5 min in seconds)