                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if math.random(100) <= 40 then
                                        TriggerServerEvent('rs-hud:Server:GainStress', math.random(1, 2))
                                    end
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if not copsCalled then
                        ...
                        TriggerServerEvent("rs-storerobbery:server:callCops", "cashier", currentRegister, streetLabel, pos)
                        copsCalled = true
                    end
                else
                    RSCore.Functions.TriggerCallback('RSCore:HasItem', function(result)
                        if result then
                            ...
                            RSCore.Functions.Notify("Het lijkt erop dat je een schroevendraaier mist..", "error")
                        end
                    end, "screwdriverset")
                end
...
function setupRegister()
    RSCore.Functions.TriggerCallback('rs-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    RSCore.Functions.TriggerCallback('rs-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('fail', function()
    if usingAdvanced then
        ...
        if math.random(1, 100) < 40 then
            ...
            TriggerEvent('inventory:client:ItemBox', RSCore.Shared.Items["lockpick"], "remove")
        end
    end
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(GetPlayerPed(-1))
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        RSCore.Functions.Notify("Je hebt je gescheurd aan de lockpick..")
    end
    lockpick(false)
end)
...
RegisterNetEvent('rs-storerobbery:client:setRegisterStatus')
AddEventHandler('rs-storerobbery:client:setRegisterStatus', function(register, bool)
    Config.Registers[register].robbed = bool
end)

RegisterNetEvent('rs-storerobbery:client:setSafeStatus')
AddEventHandler('rs-storerobbery:client:setSafeStatus', function(safe, bool)
    Config.Safes[safe].robbed = bool
end)

RegisterNetEvent('rs-storerobbery:client:robberyCall')
AddEventHandler('rs-storerobbery:client:robberyCall', function(type, key, streetLabel, coords)
