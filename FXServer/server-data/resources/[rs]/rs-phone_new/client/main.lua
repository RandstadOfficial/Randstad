...
AddEventHandler('rs-phone_new:client:AddRecentCall', function(CallData, type, time)
...
Citizen.CreateThread(function() 
    ...
    isLoggedIn = true
    print('2')
    RSCore.Functions.TriggerCallback('rs-phone_new:server:GetPhoneData', function(pData)
        print('1')
        SendNUIMessage({ 
...
AddEventHandler('RSCore:Client:OnPlayerLoaded', function()
    ...
    RSCore.Functions.TriggerCallback('rs-phone_new:server:GetUserContacts', function(PlayerContacts) 
        PhoneData.Contacts = PlayerContacts
    end)
    RSCore.Functions.TriggerCallback('rs-phone_new:server:GetAppAlerts', function(Applications) 
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[ChatNumber] ~= nil then
        if PhoneData.Chats[ChatNumber].messages[ChatDate] ~= nil then
            ...
            TriggerServerEvent('rs-phone_new:server:UpdateMessages', PhoneData.Chats[ChatNumber].messages, ChatNumber, false)
        else
            ...
            TriggerServerEvent('rs-phone_new:server:UpdateMessages', PhoneData.Chats[ChatNumber].messages, ChatNumber, true)
        end
    else
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    RSCore.Functions.TriggerCallback('rs-phone_new:server:PayInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    RSCore.Functions.TriggerCallback('rs-phone_new:server:DeclineInvoice', function(CanPay, Invoices)
...
RegisterNetEvent('rs-phone_new:server:TransferMoney')
AddEventHandler('rs-phone_new:server:TransferMoney', function(amount, senderIban)
    if PhoneData.isOpen then
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "RBank", text = "Er is &euro;"..amount.." bijgeschreven!", icon = "fas fa-university", color = "#8c7ae6", }, })
        SendNUIMessage({ action = "UpdateBank", NewBalance = PhoneData.PlayerData.money.bank })
    else
        SendNUIMessage({ action = "Notification", NotifyData = { title = "RBank", content = "Er is &euro;"..amount.." bijgeschreven!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)
...
RegisterNUICallback('CallContact', function(data, cb)
    RSCore.Functions.TriggerCallback('rs-phone_new:server:GetCallState', function(CanCall, IsOnline)
        local status = { 
            cc = CanCall, 
            io = IsOnline,
...
CallContact = function(CallData)
    ...
    TriggerServerEvent('rs-phone_new:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId)
    ...
    for i = 1, Config.CallRepeats + 1, 1 do
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                ...
                TriggerServerEvent('rs-phone_new:server:AddRecentCall', "outgoing", PhoneData.CallData)
                CancelCall()
                break
            end
        else
...
AddEventHandler('rs-phone_new:client:GetCalled', function(CallerNumber, CallId)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
    ...
    for i = 1, Config.CallRepeats + 1, 1 do
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                ...
                TriggerServerEvent('rs-phone_new:server:AddRecentCall', "missed", PhoneData.CallData)
                SendNUIMessage({
                    action = "IncomingCallAlert",
                    CallData = PhoneData.CallData.TargetData,
                    Canceled = true,
