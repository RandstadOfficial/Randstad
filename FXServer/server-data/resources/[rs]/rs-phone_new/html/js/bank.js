...
$(document).on('click', '.bank-app-account', function(e){
    ...
    RS.Phone.Notifications.Add("fas fa-university", "RBank", "Bankrekening nr. gekopieerd!", "#badc58", 1750);
});
...
RS.Phone.Functions.DoBankOpen = function() {
    ...
    $(".bank-app-account-balance").html("&euro; "+RS.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance").data('balance', RS.Phone.Data.PlayerData.money.bank);

    $(".bank-app-loaded").css({"display":"none", "padding-left":"30vh"});
    $(".bank-app-accounts").css({"left":"30vh"});
    $(".rsbank-logo").css({"left": "0vh"});
    $("#rsbank-text").css({"opacity":"0.0", "left":"9vh"});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
        if (amountData >= amount) {
            $.post('http://rs-phone_new/TransferMoney', JSON.stringify({
                iban: iban,
                amount: amount
                ...
                if (CanTransfer) {
                    ...
                    RS.Phone.Notifications.Add("fas fa-university", "RBank", "Je hebt &euro; "+amount+",- overgemaakt!", "#badc58", 1500);
                } else {
                    RS.Phone.Notifications.Add("fas fa-university", "RBank", "Je hebt niet genoeg saldo!", "#badc58", 1500);
                }
            })
            RS.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
        } else {
        ...
        RS.Phone.Notifications.Add("fas fa-university", "RBank", "Vul alle velden in!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://rs-phone_new/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                RS.Phone.Notifications.Add("fas fa-university", "RBank", "Je hebt &euro;"+InvoiceData.amount+" betaald!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('http://rs-phone_new/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        invoiceId: InvoiceData.invoiceid,
    }));
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        RS.Phone.Notifications.Add("fas fa-university", "RBank", "Er is geen IBAN gebonden aan dit contact!", "#badc58", 2500);
    }
    RS.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});