...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('http://rs-phone_new/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    RS.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});

RS.Phone.Notifications.LoadTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        $(".twitter-home-tab").html("");
        $.each(Tweets, function(i, Tweet){
            var TwtMessage = RS.Phone.Functions.FormatTwitterMessage(Tweet.message);
            var today = new Date();
...
RS.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets.length > 0) {
...
RS.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('http://rs-phone_new/PostNewTweet', JSON.stringify({
        ...
        RS.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
        RS.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Vul een bericht!", "#1DA1F2");
    }
});
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://rs-phone_new/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            RS.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    RS.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag')+ " gekopieerd!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

RS.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
        $(".twitter-hashtags").html("");
        $.each(hashtags, function(i, hashtag){
            var Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Niet trending in Randstad</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' Tweets</div> </div>';
...
RS.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    RS.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
