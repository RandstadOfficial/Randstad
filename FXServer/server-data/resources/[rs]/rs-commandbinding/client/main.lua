...
RegisterNetEvent('rs-commandbinding:client:openUI')
AddEventHandler('rs-commandbinding:client:openUI', function()
    openBindingMenu()
end)
        ...
        if isLoggedIn then
            for k, v in pairs(availableKeys) do
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    local keyMeta = RSCore.Functions.GetPlayerData().metadata["commandbinds"]
                    local args = {}
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            TriggerServerEvent('RSCore:CallCommand', keyMeta[v]["command"], args)
                            keyPressed = true
                        else
                            RSCore.Functions.Notify('Er is nog niks aan ['..v..'] gebind, /binds om een commando te binden', 'primary', 4000)
                        end
                    else
                        RSCore.Functions.Notify('Je hebt nog geen commands gebind, /binds om een commando te binden', 'primary', 4000)
                    end
                end
            end
...
RegisterNUICallback('save', function(data)
    ...
    RSCore.Functions.Notify('Commandbindings zijn opgeslagen!', 'success')

    TriggerServerEvent('rs-commandbinding:server:setKeyMeta', keyData)
end)