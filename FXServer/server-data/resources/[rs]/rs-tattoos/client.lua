...
function GetNaked()
	...
	if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
		...
		TriggerEvent('rs-clothing:client:loadOutfit', outfitMan)
	elseif GetEntityModel(PlayerPedId()) == `mp_f_freemode_01` then
		...
		TriggerEvent('rs-clothing:client:loadOutfit', outfitFemale)
	else
		RSCore.Functions.TriggerCallback('rs-tattoos:GetPlayerTattoos', function(tattooList)
			if tattooList then
				ClearPedDecorations(PlayerPedId())
...
function BuyTattoo(collection, name, label, price)
	RSCore.Functions.TriggerCallback('rs-tattoos:PurchaseTattoo', function(success)
		if success then
...
function RemoveTattoo(name, label)
	...
	TriggerServerEvent("rs-tattoos:RemoveTattoo", currentTattoos)
	RSCore.Functions.Notify("Je hebt tattoo "..GetLabelText(label).. " verwijderd.", "error", 2500)
end
