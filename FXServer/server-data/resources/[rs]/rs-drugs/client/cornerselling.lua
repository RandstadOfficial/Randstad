...
RegisterNetEvent('rs-drugs:client:cornerselling')
AddEventHandler('rs-drugs:client:cornerselling', function(data)
    RSCore.Functions.TriggerCallback('rs-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if result ~= nil then
            availableDrugs = result

            if not cornerselling then
                cornerselling = true
                RSCore.Functions.Notify('Standje cornerselling ingeschakeld.. yeet')
                startLocation = GetEntityCoords(GetPlayerPed(-1))
                -- TaskStartScenarioInPlace(GetPlayerPed(-1), "CODE_HUMAN_CROSS_ROAD_WAIT", 0, false)
            else
                cornerselling = false
                RSCore.Functions.Notify('Standje cornerselling uitgeschakeld.. yeet')
                -- ClearPedTasks(GetPlayerPed(-1))
            end
        else
            RSCore.Functions.Notify('Je hebt geen drugs op je..', 'error')
        end
    end)
end)

function toFarAway()
    RSCore.Functions.Notify('Je bent teveel aan het lopen.. begin maar weer opnieuw!', 'error')
    cornerselling = false
    hasTarget = false
    busySelling = false
    startLocation = nil
...
RegisterNetEvent('rs-drugs:client:refreshAvailableDrugs')
AddEventHandler('rs-drugs:client:refreshAvailableDrugs', function(items)
...
function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                TriggerServerEvent('rs-drugs:server:robCornerDrugs', availableDrugs[drugType].item, bagAmount)
                RSCore.Functions.Notify('Je bent beroofd van '..bagAmount..' zakje(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 then
                    RSCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '~g~E~w~ '..bagAmount..'x '..currentOfferDrug.label..' voor €'..randomPrice..'? / ~g~G~w~ Aanbod afwijzen')
                    if IsControlJustPressed(0, Keys["E"]) then
                        RSCore.Functions.Notify('Aanbod geaccepteerd!', 'success')
                        TriggerServerEvent('rs-drugs:server:sellCornerDrugs', availableDrugs[drugType].item, bagAmount, randomPrice)
                        hasTarget = false
                    ...
                    if IsControlJustPressed(0, Keys["G"]) then
                        RSCore.Functions.Notify('Aanbod geweigerd!', 'error')
                        hasTarget = false
