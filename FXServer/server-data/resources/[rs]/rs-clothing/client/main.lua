...
AddEventHandler('RSCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("rs-clothes:loadPlayerSkin")
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    gender = "male"
                                    if RSCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    RSCore.Functions.TriggerCallback('rs-clothing:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNetEvent('rs-clothing:client:openOutfitMenu')
AddEventHandler('rs-clothing:client:openOutfitMenu', function()
    RSCore.Functions.TriggerCallback('rs-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('rs-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('rs-clothing:client:openMenu')
AddEventHandler('rs-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('rs-clothes:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('rs-clothing:client:reloadOutfits')
AddEventHandler('rs-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('rs-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
function SaveSkin()
	...
	TriggerServerEvent("rs-clothing:saveSkin", model, clothing)
end

RegisterNetEvent('rs-clothes:client:CreateFirstCharacter')
AddEventHandler('rs-clothes:client:CreateFirstCharacter', function()
...
AddEventHandler('rs-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('rs-clothing:client:loadOutfit')
AddEventHandler('rs-clothing:client:loadOutfit', function(oData)
    local ped = GetPlayerPed(-1)
    ...
    if data["accessory"] ~= nil then
        if RSCore.Functions.GetPlayerData().metadata["tracker"] then
...
RegisterNetEvent("rs-clothing:client:adjustfacewear")
AddEventHandler("rs-clothing:client:adjustfacewear",function(type)
    if RSCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
