...
RentVehicle = function(selectedVehicle)
    ...
    if IsPedInAnyVehicle(ped) then
        RSCore.Functions.Notify('Dit kan alleen te voet..', 'error')
        return
    end

    local vehiclePlate = "RSD"..math.random(1, 100)
    TriggerServerEvent('rs-vehiclerental:server:SetVehicleRented', vehiclePlate, true, Config.RentalVehicles[selectedVehicle])

    RentedVehicleData = selectedVehicle
end

RegisterNetEvent('rs-vehiclerental:server:SpawnRentedVehicle')
AddEventHandler('rs-vehiclerental:server:SpawnRentedVehicle', function(vehiclePlate, vehicleData)
    local ped = GetPlayerPed(-1)
    local coords = {
...
ReturnVehicle = function()
    ...
    RSCore.Functions.Notify('Je hebt geen borg open staan..', 'error')
end
...
AcceptReturn = function()
    ...
    if noSpace(VehiclePlate) ~= noSpace(RentedVehiclePlate) then
        RSCore.Functions.Notify('Dit is geen gehuurd voertuig..', 'error')
    else
        TriggerServerEvent('rs-vehiclerental:server:SetVehicleRented', RentedVehiclePlate, false, Config.RentalVehicles[RentedVehicleData])
        RSCore.Functions.DeleteVehicle(CurrentVehicle)
