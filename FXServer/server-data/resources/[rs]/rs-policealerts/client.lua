...
RegisterNetEvent('rs-policealerts:ToggleDuty')
AddEventHandler('rs-policealerts:ToggleDuty', function(Duty)
    PlayerJob.onduty = Duty
end)

RegisterNetEvent('rs-policealerts:client:AddPoliceAlert')
AddEventHandler('rs-policealerts:client:AddPoliceAlert', function(data, forBoth)
    if forBoth then
...
RegisterNUICallback('SetWaypoint', function(data)
    ...
    RSCore.Functions.Notify('GPS ingesteld!')
    SetNuiFocus(false, false)
    SetNuiFocusKeepInput(false, false)
    MouseActive = false
end)
