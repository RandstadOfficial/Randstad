...
$(document).ready(function(){
    ...
    window.addEventListener('message', function(event){
        ...
        if (eventData.action == "phone") {
            if (!eventData.task) {
                if (eventData.open == true) {
                    rsPhone.Open(eventData.cid)
                    rsPhone.setupPhoneApps(eventData.apps)
        ...
        if (eventData.task == "callScreen") {
            if (eventData.callData.inCall || eventData.callData.outgoingCall || eventData.callData.incomingCall) {
                rsPhone.CallScreen(event.data.callData)
            }
        }

        if (eventData.task == "setPhoneMeta") {
            rsPhone.setPhoneMeta(eventData.pMeta, eventData.pData.charinfo.phone);
        }

        if (eventData.task == "setUserContacts") {
            rsPhone.setupPlayerContactInfo(eventData.pData.charinfo.firstname, eventData.pData.charinfo.lastname, eventData.pData.charinfo.phone);
            rsPhone.setupPlayerContacts(eventData.pContacts);
        }

        if (eventData.task == "setupBankData") {
            rsPhone.setBankData(eventData.pData)
        }

        if (eventData.task == "updateTime") {
            rsPhone.updateTime(eventData.time)
        }

        if (eventData.task == "getVehicles") {
            rsPhone.setGarageVehicles(eventData.vehicles)
        }

        if (eventData.task == "setupMail") {
            rsPhone.setupUserMails(eventData.mails)
        }

        if (eventData.task == "setupTweets") {
            rsPhone.SetupTweets(eventData.tweets);
        }

        if (eventData.task == "phoneNotify")  {
            rsPhone.Notify(eventData.title, eventData.type, eventData.text, 3500);
        }

        if (eventData.task == "setupAds") {
            rsPhone.SetupAdverts(eventData.ads);
        }

        if (eventData.task == "phoneNotification") {
            rsPhone.phoneNotification(eventData.message)
        }

        if (eventData.task == "newTweet") {
            rsPhone.SetupTweets(eventData.tweets);
            rsPhone.Notify('<i class="fab fa-twitter" style="position: relative; padding-top: 3px;"></i> Twitter', 'tweet', '@'+eventData.sender + ' heeft een tweet geplaatst!')
        }

        if (eventData.task == "setupCompanies") {
            rsPhone.SetupCompanies(eventData.companies);
        }

        if (eventData.task == "newAd") {
            rsPhone.SetupAdverts(eventData.ads);
            rsPhone.Notify('<i class="fas fa-ad" style="position: relative; padding-top: 3px;"></i> Advertentie', 'advert', eventData.sender + ' heeft een advertentie geplaatst!')
        }

        if (eventData.task == "updateChat") {
            rsPhone.UpdateChat(eventData.messages, eventData.number)
        }

        if (eventData.task == "updateChats") {
            $.post('http://rs-phone/getMessages', JSON.stringify({}), function(messages){
                rsPhone.SetupChat(messages)
            });
        }

        if (eventData.task == "newMessage") {
            rsPhone.Notify('<i class="fas fa-envelope" style="position: relative; padding-top: 7px;"></i> Berichten', 'success', eventData.sender + ' heeft een bericht gestuurd!')
        }

        if (eventData.task == "newPoliceAlert") {
            rsPhone.AddPoliceAlert(eventData);
        }
    ...
    $('.notify-btn').change(function() {
        if (allowNotifys == null) {
            if (myNotify) {
                allowNotifys = true;
                rsPhone.toggleNotify(true)
            } else {
                allowNotifys = false;
                rsPhone.toggleNotify(false)
            }
        } else {
            if (!allowNotifys) {
                allowNotifys = true;
...
$(document).on('click', '.app', function(e){
    ...
    if (pressedApp.app == "contacts") {
        ...
        $.post('http://rs-phone/getCharacterData', JSON.stringify({}), function(charinfo){
            rsPhone.SetupPoliceApp(charinfo)
        });
...
rsPhone.SetupPoliceApp = function(data) {
...
rsPhone.AddPoliceAlert = function(data) {
...
rsPhone.SetupChat = function(chats) {
...
rsPhone.UpdateChat = function(messages, number) {
...
$(document).on('click', '.sms-contact', function(){
    ...
    $.post('http://rs-phone/doesChatExists', JSON.stringify({
        ...
        if (cbData != "") {
            rsPhone.OpenExistingChat(cbData)
        } else {
            rsPhone.OpenNewChat(contactData)
        }
    });
...
rsPhone.OpenExistingChat = function(messageData) {
    $('.chat-window').css({"display":"block"}).animate({top: "3%",}, 250);
...
rsPhone.OpenNewChat = function(data) {
    $('.chat-window').css({"display":"block"}).animate({top: "3%",}, 250);
...
$(document).on('click', '.submit-contact-btn', function(e){
    ...
    if(contactName != "" || contactNum != "") {
        if (!isNaN(contactNum)) {
            ...
            rsPhone.Notify('Contacten', 'success', contactName+" ("+contactNum+") is toegevoegd aan je contacten.")
            $.post('http://rs-phone/addToContact', JSON.stringify({
                contactName: contactName,
                contactNum: contactNum
            }))
        } else {
            rsPhone.Notify('Contacten', 'error', 'Het telefoon nummer moet bestaan uit cijfers.')
        }
    } else {
        rsPhone.Notify('Contacten', 'error', 'Je hebt niet alle contactgegevens ingevuld.')
    }
});
    ...
    if (ibanVal != "" && amountVal != "") {
        if (!isNaN(amountVal)) {
            if (balance - amountVal < 0) {
                rsPhone.Notify('RBank', 'error', 'Je hebt niet genoeg saldo', 3500);
            } else {
                if (amountVal > 0) {
                    $('.transfer-money-container').css({"display":"block"}).animate({top: "103%",}, 250, function(){
                        $('.transfer-money-container').css({'display':'none'});
                        rsPhone.Notify('RBank', 'success', 'Je hebt € '+amountVal+' overgemaakt naar '+ibanVal+'!', 3500);
                    ...
                    rsPhone.Notify('RBank', 'error', 'Hoeveelheid mag niet lager zijn dan 0', 3500);
                }
            }
        } else {
            rsPhone.Notify('RBank', 'error', 'De hoeveelheid moet bestaan uit cijfers.', 3500)
        }
    } else {
        rsPhone.Notify('RBank', 'error', 'Je hebt niet alle gegevens ingevuld.', 3500)
    }
});
...
rsPhone.setupPhoneApps = function(apps) {
    $('.app-slot').html("");
...
rsPhone.Log = function(log) {
    console.log(log)
}

rsPhone.toggleNotify = function(bool) {
    allowNotifys = bool;
    if (bool) {
...
rsPhone.succesSound = function() {
    $.post('http://rs-phone/succesSound');
}

rsPhone.errorSound = function() {
    $.post('http://rs-phone/errorSound');
}

rsPhone.setPhoneMeta = function(phoneMeta, phoneNumber) {
    rsPhone.setPlayersNotification(phoneMeta.settings.notification)
    rsPhone.setPlayersBackground(phoneMeta.settings.background)
    rsPhone.setPlayersPhoneNumber(phoneNumber)
}

rsPhone.setPlayersNotification = function(allow) {
    myNotify = allow;
    if (allow) {
...
rsPhone.setPlayersPhoneNumber = function(num) {
    $('.phone-num').html(num);
}

rsPhone.setPlayersBackground = function(background) {
...
rsPhone.updateTime = function(time) {
    $("#time").html(time.hour+"."+time.minute);
}

rsPhone.setupPlayerContactInfo = function(firstName, lastName, phoneNumber) {
...
$(document).on('click', '.change-contact-btn', function(e){
    ...
    if($(".edit-number-input").val() != "" || $(".edit-contactname-input").val() != "") {
        if (!isNaN($(".edit-number-input").val())) {
            ...
            rsPhone.Notify('Contacten', 'success', 'Contact opgeslagen');
            $('.edit-contact-container').css({"display":"block"}).animate({top: "103%",}, 250, function(){
                $(".edit-contact-container").css({"display":"none"});
            });
        } else {
...
rsPhone.setupPlayerContacts = function(contacts) {
    $(".contact-list").html("");
...
rsPhone.Notify = function(title, type, message, wait) {
    // if (allowNotifys) {
...
rsPhone.setBankData = function(playerData) {
...
rsPhone.setGarageVehicles = function(vehicles) {
...
rsPhone.openMail = function(mailData) {
...
$(document).on('click', '.send-new-tweet', function(){
    ...
    if (tweetMessage != "") {
        ...
        rsPhone.Notify('Twitter', 'error', 'Je kan geen legen tweet versturen...')
    }
});
...
$(document).on('click', '.send-new-ad', function(){
    ...
    if (advertMessage != "") {
        ...
        rsPhone.Notify('Advertenties', 'error', 'Je kan geen legen tweet versturen...')
    }
});
...
rsPhone.phoneNotification = function(message) {
...
rsPhone.SetupTweets = function(tweets) {
    $('.twitter-tweets').html("");
    if (tweets != undefined) {
...
rsPhone.SetupCompanies = function(companies) {
...
rsPhone.SetupAdverts = function(ads) {
    $('.advert-ads').html("");
    if (ads != undefined) {
...
rsPhone.CallScreen = function(callData) {
